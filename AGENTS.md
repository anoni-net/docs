...
| 導覽列圖示 | `docs/hooks/nav_icon_sprite.py`、`test_nav_icon_sprite.py` | 文章 front matter 的 `icon:` 原本由 Material 以 inline SVG 畫進導覽列，而導覽列在每一頁都是整棵樹，每一頁都揹著整站的圖示。這支 hook 在建置時把用到的圖示收成 `assets/nav-icons.<hash>.svg`，導覽連結改用 `<use>` 指向它，外層 `<svg>` 的開頭標籤原封不動，外觀逐像素比對過相同。2026-09 量 zh-TW，HTML 壓縮後從 17.3 MB 降到 11.8 MB。不覆寫 `nav-item.html`，Material 停止維護之後換框架時少一份要處理的樣板。`offline_index.py` 把 `<use href>` 算成頁面資產，sprite 因此落進索引的 `shell` 由 service worker 預快取，`check_precache.mjs` 的反向檢查也涵蓋它，漏了的話離線時整條側邊欄的圖示是空白的。認不得的圖示保留 inline，部落格文章頁上返回、日期、分類那幾個來自部落格樣板，不在 front matter 裡，照原樣留著 |
...
| 更新日誌 KEV 對照 | `check_changelog_kev.py`、`test_check_changelog_kev.py` | 抓 CISA 的已知遭利用漏洞目錄（KEV），把近 60 天新增、屬於更新日誌追蹤的產品的 CVE 跟 zh-TW 頁面對一遍，列出已被利用、頁面還沒寫的。上游的利用標注不一定出現在我們讀的那份公告裡，2026 年 9 月漏過 Pixel 數據機與 Windows IKE 兩筆，都是回頭對 KEV 才發現。產品對到哪幾頁寫在 `RULES`，對照表寫錯的症狀是一筆都不報，看起來像什麼都沒漏，所以測試用假資料把每一類各走一次。由 `changelog-kev.yml` 每週執行，有漏的就開或更新一張 issue，不改內容 |
| 部署 | `cf_purge.py`、`test_cf_purge.py` | 建置完把產物映射回網址，分批並行清除 Cloudflare 快取（每批 30 條，同時 6 批）。JS 模組與字型另外連同 `Origin` 標頭再清一次：瀏覽器抓 `<script type="module">` 與字型時帶著 `Origin`，Cloudflare 存成另一份快取，只依網址清不到。2026-09-23 之前一直漏清，地球儀部署後讀者取得的是新資料配舊程式。測試由 [`tools-tests.yml`](./.github/workflows/tools-tests.yml) 在 PR 觸發 |
