...
`bytes` 是渲染後 HTML 的位元組數，不含圖片與 CSS/JS。頁面自己引用的圖與程式列在
`assets`，大小在 `assetBytes` 與全域的 `assets` 表，讀者勾一頁時兩者一起下載，管理頁
去重之後加總，顯示的就是實際會增加的量。每一頁都載入的那批不算在個別頁面頭上，
寫在 `shell` 交給 service worker 預快取。
"""
