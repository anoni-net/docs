...
还没有 passkey 的话，先到[passkey 钥匙](passkey.md)创建一把，或直接在下面创建，两条路结果一样，不确定就到钥匙页照步骤做。passkey 是什么、限制在哪，见[什么是 passkey](../tools/what-is-passkey.md)。
...
- 这份清单只记你勾了什么跟哪一天勾的。passkey 丢了、清单无法解开，重勾一次就好，这里没有备援密钥那一步。
...
- 这里的加密强度等于保管 passkey 的地方：要防的人能接触到这台设备、又知道解锁密码或密码管理器的主密码，他一样看得到你勾了什么。那种处境先读[家暴受害者的数位准备](../scenarios/domestic-violence.md)，考虑不在这台设备上留任何准备的痕迹。要防的人是同住的伴侣、执法单位或国家级时，别在这台设备上创建钥匙，也别存清单。这一页没有只看不存的模式，项目本身在上面两篇情境文章里都读得到，用看的就好。怎么判断见[威胁模型清单的「答案预设不存」](threat-model.md#答案预设不存)。
