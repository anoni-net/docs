...
如果那份文件是二十份带不同标记的副本之一，这个动作等于把消息来源交给正在找他的人。每一步都做得很专业，来源却在查证那一刻被烧掉。
...
反方向也成立。媒体内部发给编辑台的稿子、公关公司发给记者群的禁运稿，同样可能带标记，用来确认谁提前放了出去。收到禁运稿的人也在这个范围里。
...
onion 地址用的是 base32，只有 `a` 到 `z` 与 `2` 到 `7`。任何西里尔字母或不可见字符都不在那个集合里，贴进 Tor Browser 多半是连不上，而不是连到别的地方。bridge line 的情况比较实在：那是一长串设置，中间夹了不可见字符会让解析出错，或在转贴过程中被截断成另一组看起来合理的参数。

真正会「连到别的地方」的手法这一页抓不到：攻击者可以自己算出一个前缀看起来很像的合法 onion 地址，那里面没有任何隐形字符，每一个字都是合法的 base32。分辨它只能逐字比对完整的 56 个字符，或从你信任的来源重新取得。
...
标签字符那一区特别适合做这件事。那个区段可以把英文一个字符对一个字符编成完全不显示的东西，长度不限，所以整段指令都塞得进去。这一页会把那一区的字符标成可疑，正常文字用不到它们。
...
零宽字符是最粗糙的一种标记方式。正因为粗糙，才容易检测。真正要追人的一方有更多不会被这一页抓到的做法：
...
还有完全不在文字里的。彩色激光打印机会在每一张打印出来的纸上留下黄点阵列，编码打印机序号与打印时间，那套东西叫 Machine Identification Code。文件经过打印再扫描之后，你手上那份的来源仍然查得到。
...
这一页的立场是：说明手法怎么运作，让收到文件的人有能力自保，但不提供产生带标记副本的功能。

不做的具体包括：把一段文字加上不可见标记的生成器、批量产出多份带不同标记副本的工具。

「载入示范文字」给的是固定的一段，不能换成你自己的文字。那一段里有两个零宽空格、一个冒充网址的西里尔字母、四个标签字符，让你看得到检测结果长什么样。教育效果需要的就是这个，而它拿不去标记真实的文件。想更完整验证检测范围的人，用测试档里的案例（`tools/test_invisible.mjs`）。

这是社群立场，写在这里是为了让日后的讨论有个可以引用的地方。

## 用 Tor Browser 查可疑内容的话

[Tor Browser 的安全等级](../tools/tor-browser-advanced.md)调到 Safest 会关掉 JavaScript，这一页就整页不动，而收到来路不明的消息正是那篇建议把等级调高的场合。

把可疑的网站与这一页分开处理：在高安全等级下复制那段文字，切回 Standard 贴进来扫，扫完再调回去。这一页不连外，扫描是在你的设备上做的。
