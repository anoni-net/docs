...
站上用得到它的有四处：[本机文件加密](age.md)、[我的准备清单](checklist.md)、[威胁模型清单](threat-model.md)的存档，以及本机文件加密里的收件人簿。后三个共用同一份 passkey 加密的暂存区。钥匙切到「passkey」之后，加密时按一次指纹就好，不用想密语也不用打字。加密时预设会同时加密给 passkey 与备援密钥，两把任何一把都开得了。备援那一把可以取消，取消之后只有 passkey 开得了，代价写在加密工具的画面上。
...
别的网站用 passkey 登录，需要的只是签个名证明是你，任何保管方式都做得到。这一页要它多做一件事，算出一把加密密钥。那个能力要保管方式另外实作才有，所以同一把 passkey 在别的网站好好的，在这里却可能算不出密钥。下面这张表列的就是哪些保管方式做得到后面这件事。


| 存放位置 | 会不会同步到其他设备 | 算得出密钥吗 |
|---|---|---|
