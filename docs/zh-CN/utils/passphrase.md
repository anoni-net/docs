...
- **刚申请密码管理器，要设一组主密码。** 那组密码是所有其他密码的门，随手打几个字应付过去很常见，但没人知道那样够不够强。用密语模式抽六个字，画面会直接告诉你熵有多少。
- **怀疑手边的电脑被人动过。** 切到「实体骰子」那一页，随机性来自你自己的手，不需要相信这台机器。
...
而查表这件事你也可以自己验。按「下载完整对照表」会给你一份纯文字档，7776 行，每行是编码加对应的字。打印出来或存起来之后，这一页就不是必要的。

几件实际操作上的事：

- 骰子要够公正。便宜的塑料骰子有偏差，赌场等级的精密骰子才真的均匀。日常用途下一般骰子已经够好，但知道这件事比不知道好。
- 不要用手机上的掷骰 App。那绕了一圈回到「要相信软件」。
- 五颗骰子一起掷比一颗掷五次快，但要记得分辨顺序，可以用不同颜色的骰子。
- 熵跟程序抽取完全相同，每掷一轮相当于从 7776 个字里抽一次，约 12.9 bits。
...
跟这一区其他工具一样，程序存进设备之后没有网络也能用。词表也会一起存下来，断网时照样抽得出密语，那正是「生成的密码没有被送出去」最直接的证明。
