...
拿摄像头拍屏幕会出现一圈一圈的水波纹，叫摩尔纹。[QR code 读取器](qr-read.md)那一页写着宁可截图也不要拍屏幕，就是因为它。这一页没得选，只能拍屏幕，所以方格大小跟距离比那一页更要紧。
...
工具会替文件算一个校验码（SHA-256）放进第一张。校验码可以想成文件的指纹，发的一端先按一次指纹跟着送过去，收的一端拼完再按一次，两个一样就代表中间一个位都没有出错。收齐之后画面上会直接显示对不对得上。

要注意的是，指纹一样只证明**文件没有在传输中坏掉**，不证明**文件是谁给的**。对方是谁，还是要用别的方法确认，例如当面核对 PGP 指纹，或是回到[已经建立信任的管道](../tools/messaging-comparison.md)再确认一次。

## 印成纸质

按下「印成纸质」会把所有的码排成 A4 版面，一页四张，每张下面标着编号。印出来的一叠纸就是那个文件。

用得上的情况跟屏幕串流不一样：

- **对面现在不在**。纸可以寄、可以托人带、可以放进抽屉，收的人几天后再扫
- **连电力都断了**。纸不需要电，那是[网络中断时](../scenarios/shutdown.md)第四级还在运作的少数载体之一
- **手上只有一台设备**。举着手机对笔记本那套要两台，印出来就不必

读回的方式跟屏幕一样，用另一台设备打开这一页的「接收」，镜头对着纸上每一张读过去。顺序不重要，每一张都自己带着编号、总张数与校验码，收齐就会拼回原文件。

### 印出来的东西一样没有加密

纸上那些方格跟屏幕上的是同一件东西，拍得到的人都读得回来。而纸比屏幕更容易被拍：它会被放在桌上、夹在文件夹里、留在复印机的纸盒。敏感的内容先用[本机文件加密](age.md)封起来再印。

### 张数的上限是 120 张

也就是 30 页。屏幕播不完只是等久一点，纸质印不完是一叠没有人会去扫的纸。

超过的时候画面会显示这个文件要印几页。把「每张数据量」调到大或极限可以少印很多，代价是方格变密，扫的时候要靠近一点。真的大的文件不适合走纸质，适合的是密钥、配置文件、一份联系清单这类小东西。

### 印出来的是矢量图

纸上的码用 SVG 生成，没有拿屏幕上的画布放大。激光打印机是 600 dpi，画布在屏幕上只有几百像素宽，放大到 78 mm 边缘会糊，而糊掉的方格边界正是解码器最先失败的地方。

一张印成 78 mm 见方，极限密度是 177 个方格，一格 0.44 mm，600 dpi 的打印机一格有十个点，手机拍单张读得回来。
...
这一页本身要先加载过。第一次打开这一页需要网络，因为程序要从站上下载。真正要在断网的场合用它，请事先在有网络的时候打开这一页并且[存成离线内容](../offline.md)，之后没有网络也可以打开。
...
浏览器能使用的蓝牙功能只设计来跟手环、体重计那类小设备交换很短的数值，规格里根本没有传文件这件事。而且网页只能扮演「主动去连别人」的那一方，没办法让别人来连自己，所以两个网页永远看不见对方。再加上 Firefox 完全没有实现这个功能，以它为基础的 Tor Browser 也就没有。
...
[我的准备清单](checklist.md)的「传到另一台」会把暂存区的密文带到这一页，直接当作要传的文件，按「开始播放」就好。另一台用这一页的接收端扫，收齐而且校验码相符时，结果旁边多一颗「导入我的准备清单」，按下去带着密文回清单页，再用同一把 passkey 解开。密文经网址片段交接，片段不会送到服务器，读完就清掉。
