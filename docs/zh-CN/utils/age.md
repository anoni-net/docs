...
加密时把钥匙切到「passkey」，就不用记密语。浏览器会跳出提示，用指纹或 PIN 同意一次，文件就加密给你的 passkey。第一次用之前先到 [passkey 钥匙](passkey.md)创建一把。「另外加一把备援密钥」预设勾着，那把公钥填在下面的栏位，passkey 丢了或换到不支持的环境时只剩它能开。你的处境不需要那条退路就取消勾选，取消之后画面上会写清楚代价。解密时工具看文件头决定要问密语、passkey 还是备援私钥，用 age 命令行加密给 `age1` 公钥的文件也能在这里贴私钥解开。passkey 模式的文件只能在这个网站解，理由见[什么是 passkey](../tools/what-is-passkey.md)。这个环境算不算得出密钥，先到[钥匙页](passkey.md)按「试解锁」确认。

备援密钥栏位与收件人栏位下面有一本收件人簿：常用的 age1 公钥取个名字存进你 passkey 加密的暂存区，跟[我的准备清单](checklist.md)同一份密文，下次选一个就填进去。存与删都要你自己按。名单本身是谁跟你往来的线索，它跟清单一样存在暂存区，安全等于密码管理器的安全。

## 用公钥加密给别人

钥匙切到「公钥」，收件人一行一个，age1 开头的 X25519 公钥与 age1pq1 开头的后量子混合公钥都收，每一位收件人都能用自己的私钥解开。要自己也能解，把自己的公钥也放一行。没有密钥的话按「生成密钥」，公钥会加进收件人，私钥只能下载成 `key.txt`，这一页不留，关掉就没了。跟 age 命令行的 `age-keygen` 生成的文件格式一样，两边可以互用。

加密完会检查文件头的段落数跟收件人数一致，查的只是数量，贴错公钥查不出来。刚生成的那把在收件人里时，顺便用它解回来比对。只用别人的公钥时本机没有东西能解回来验证，先寄一份测试文件请对方确认解得开，再寄正式的。

解密时文件头只有公钥段落，工具会要私钥：贴进栏位，或选 `key.txt`，把 `key.txt` 拖进拖放区也认得。

## 密语就是全部

密语模式没有密钥要管，代价是加密的强度完全等于密语的强度。scrypt 让每一次猜测都要花零点几秒，能拖慢暴力尝试，但对一个好猜的密语没有帮助。六个词以上、不是名言或歌词、没有在别处用过，理由见 [Asian Diceware 密语字典](../tools/asian-diceware.md)。忘了密语没有任何人能救，备份的密语自己也要备份，写在纸上放在跟备份不同的地方。

## 注意

- 整份文件在内存里处理，超过 200 MB 会先挡下。大的备份先切小，或直接用命令行工具，安装方式见[什么是 age](../tools/what-is-age.md)。
