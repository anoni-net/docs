...
- **停车缴费机上贴着一张 QR code**，看起来跟原本的缴费码没两样。扫之前先在这一页确认里面是不是真的付款页面，主机名跟官方域名对不上就知道那张贴纸有问题。
...
上面那一格原始内容不遮，一字不差，目的是让你能确认解出来的到底是什么。所以要把这一页截图给别人之前，先看清楚那一格里有没有密码。`otpauth:` 的密钥是唯一的例外，字段表完全不列它。

## 解出来的东西还可以往下处理

如果是网址，这一页会数出它带了几个跟踪参数，[网址清理器](clean-url.md)可以拿掉那几个再分享。

如果是一段文字，[隐形字符检测](invisible.md)可以看看里面有没有藏东西。

坐标的精度那一栏是换算出来的：小数点后每多一位，指到的范围就缩小十倍。小数第四位在纬度方向大约是 11 米，指得到一栋建筑。这跟[浏览器透露了什么](leaks.md)那一页说明的位置泄漏是同一件事的两面。

## 用 Tor Browser 扫可疑的码

[Tor Browser 的安全等级](../tools/tor-browser-advanced.md)调到 Safest 会关掉 JavaScript，这一页就整页不动，而收到来路不明的 QR code 正是那篇建议把等级调高的场合。

先把图片存下来，切回 Standard 再读，读完调回去。这一页不连外，图片是在你的设备上解的。

## 读不出来的时候

读不到的时候这一页不会只试一次。它会换几种方式再试：先用缩小的版本，接着裁掉外围把中间放大，再用原始尺寸，最后拉开黑白对比。多数照片在第一种就解完了，只有解不出来的才会往下走。

还是读不到的话，这几件事按效果排序：

- **裁掉 QR code 以外的部分**，这个最有效。手机拍的照片动辄四千像素宽，墙上那张码在画面里可能只占一小块，裁过之后它就占满整张图
- **靠近一点重拍**，让码占画面一半以上
- **避开反光**，光斑盖住定位图样就没得救
