...
多数人一开始就想切太细，层数越多，每天要记得的规则越多，撑不过三个月就会开始便宜行事，然后在某次疲劳的时候用错账号。先回到 [威胁模型如何建立](./threat-model.md) 的三个问题，再决定层数。
...
长期观察一个账号的活跃时段，能推出所在时区、上下班时间、周末习惯。两层如果同步在深夜安静、早上八点活跃，作息模式本身就是识别特征。出国时两层同时跳到新时区更明显。

处理方式是接受这件事挡不掉，改成减少可观察的样本：次要层降低发文频率、用排程工具错开发布时间、旅行期间直接让次要层安静。
