...
Google 在 2019 年说明过落差，用户无法清除指纹，因此无法控制自己的信息怎么被收集，「我们认为这破坏了用户的选择权，是错的」[^google2019]。同一家公司后来的政策走向见下方〈各家浏览器现在做到哪里〉。
...
成立条件是覆盖范围要完整。漏掉的字段会变成稳定的锚点，让随机化过的部分失去意义。
...
**Firefox** 在 `145` 版补上第二阶段防护。已知的指纹脚本沿用增强型跟踪保护（Enhanced Tracking Protection）的清单拦截，未列名的则改用限制 API 输出的方式处理，两项都在无痕窗口与增强型跟踪保护的严格模式默认开启。Mozilla 表示被判定为唯一的用户比例因此接近减半，全局默认开启仍在进行中[^mozilla]。另外一个开关 `privacy.resistFingerprinting` 沿用 Tor Browser 的一致化路线，默认关闭，需要自行到 `about:config` 开启。
...
政策方向另有一项变化值得记住。Google 在 2024 年 12 月 18 日公告修改广告平台政策，移除禁止使用设备指纹的条款，2025 年 2 月 16 日生效[^policy]。英国信息专员办公室（Information Commissioner's Office, ICO）隔日回应，称该决定不负责任，并指出使用指纹识别的业者仍须证明符合数据保护法在透明、同意与可删除等方面的要求[^ico]。
...
读结果的时候有两件事要记得。测试站的样本来自主动造访的人，判定为唯一的比例会偏高。真正有用的是逐项看哪几个字段识别力最高，那些字段是你调整之后收益最大的地方。
...
挡不掉的部分：你主动登录的账号。指纹防护处理的是不具名的跨站关联，登录行为本身直接告诉网站你是谁。
...
挡不掉你自愿交出去的信息。换设备与换工具改变的是关联，你在单一网站上留下的内容没有因此减少。
