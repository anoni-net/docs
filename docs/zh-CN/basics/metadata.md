...
上面那张图是同一件事的另一种画法。信封外面那一圈栏位，收信的人、经手的服务器、掌握线路的一方都看得到，不需要任何解密动作。右边三条是光凭那一圈就能得到的结论。
...
注意：就算内容是端对端加密，上面这些通常不加密。Signal 算是少数刻意减少这类 metadata 收集的工具，但仍无法完全消除（例如注册仍要绑手机号码，本身就是一个身份线索）。Signal 从 2024 年起提供用户名（username），让你可以不向联系人公开电话号码、也能关掉「用手机号码被搜索到」的功能，降低一部分暴露。2026 年 9 月起，Android 的 beta 版开始测试付费、不绑手机号的注册方式 Signal Login，付款端仍会留下购买记录（取舍见[Signal 免手机号注册与付款记录](../blog/posts/2026-signal-login-numberless.md)）。正式版推出之前，多数账号仍然绑在手机号码上，对需要完全匿名的情境仍是限制。
...
- 上传照片前先去 EXIF（手机内建分享时可选「不分享位置」）。要自己确认拿掉了什么，用[照片 metadata 清除器](../utils/strip-metadata.md)，它在浏览器里改、文件不上传，并且会把每一段的去留列出来
...
- DNS 改用支持加密查询的服务（DoH/DoT）。这一步换掉的是谁收得到你的查询，连接本身不会因此变匿名（见 [常被误认为匿名的网络](../advanced/mistaken-for-anonymity.md)）
