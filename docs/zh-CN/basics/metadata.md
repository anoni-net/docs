...
近年 Encrypted Client Hello（ECH，把 TLS 握手里的目标域名也一起加密的机制）开始改变 SNI 这一块。Firefox 119、Chrome 117 之后默认支持，Cloudflare 等大型 CDN 也默认开启，SNI 可以不再以明文出现。但它需要服务器端支持、在加密 DNS（DoH）环境下效果最好，也可能被中间设备阻挡，所以还不能假设每条连线的 SNI 都被保护。
