...
想留在台湾境内的话，TWNIC 营运的 Quad101 是目前唯一的公共选项，地址 `101.101.101.101` 与 `101.102.103.104`，启用 DNSSEC 验证[^quad101]。用这页的四项准则看它，有三件事值得先知道。
...
记录保留的时间也比境外几家长，官方说明是仅保留最低限度的查询数据，临时性记录最长 30 天，内容包含来源与目的 IP、端口、查询类型、域名、时间戳与回应数据，长期保留的则是排除个人信息的抽样[^quad101-privacy]。对照 Cloudflare 对 `1.1.1.1` 承诺的 25 小时内删除，差距在这里。

加密端点则没有公布，官网说明支持 DoH 与 DoT，却没有给出主机名称或网址[^quad101-terms]。照这页前面的逻辑，只填 `101.101.101.101` 这个 IP 得到的是明文查询，要用加密就得先跟 TWNIC 问到端点。

选 Quad101 等于把数据留在台湾、交给本地机构保管，代价是接受一组包含法令遵循在内的过滤，以及比境外几家长的记录保留期。这个取舍没有标准答案，但它跟表上另外三家不是同一种产品，选之前先知道差在哪里。
