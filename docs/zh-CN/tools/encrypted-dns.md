...
DoH 常被说成抗封锁能力最好，这句话要限缩。它挡住的只有按端口全面封锁这一种手法，DoH 连接自己的 TLS 握手里带着 resolver 的域名，按 SNI 或按 IP 封锁特定 resolver 一样做得到。在会针对加密 DNS 动手的网络里，别把 DoH 当成一定连得上。
...
这张表有两个容易踩到的地方。第一，Quad9 对外主打的 `9.9.9.9` 是**有过滤**的那一组，不想要过滤需改用 `9.9.9.10`，默认值跟 Cloudflare 刚好相反。第二，Quad9 的 `9.9.9.11` 多出来的 ECS（EDNS Client Subnet）会把你 IP 地址的一段前缀附在查询里送给权威服务器，用途是让 CDN 选近一点的节点，代价是原本只有 resolver 知道的来源信息，上游也拿得到一部分，以隐私而言那是减项。
...
它没有不过滤的版本，官网列出的过滤范围除了钓鱼、恶意软件、僵尸网络 C&C 这三类，还有一类是「违反适用法令之内容域名」[^quad101-terms]，这一类比前三类宽得多，而且跟其他家一样没有公布清单，你无法事先知道涵盖哪些域名。要拿它来判断网络有没有被干扰，或者要跑 OONI Probe，这个性质会直接影响结果。
...
加密端点则没有公布，官网说明支持 DoH 与 DoT，却没有给出主机名称或网址[^quad101-terms]。照这页前面的逻辑，只填 `101.101.101.101` 这个 IP 得到的是明文查询，要用加密就得先跟 TWNIC 问到端点。
