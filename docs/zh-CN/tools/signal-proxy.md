...
Signal 的服务器地址固定，封锁起来相当直接。常见手法是把 Signal 的域名从 DNS 响应里拿掉、把服务器 IP 放进黑名单，或在 TLS 握手时读 SNI 栏位认出目的地再切断连接。App 的表现就是一直停在「连接中」，消息送不出去。

几个已知的封锁时间点：

- **伊朗**：2021 年 2 月起大规模封锁，Signal 为此在官方博客[向社区征求 proxy 志愿者](https://signal.org/blog/help-iran-reconnect/){target="_blank"}。
- **中国**：2021 年 3 月起需要 proxy 或其他绕行工具才能使用，App Store 中国区也已下架。
...
Signal 官方的做法叫做 TLS proxy，跟一般的 HTTP proxy 有明显差别。连上一般 HTTP proxy 时，客户端会先送出一段明文的 `CONNECT` 请求说明要连去哪里，审查系统看到这段就知道有人在用代理。Signal TLS Proxy 没有这个步骤，整段连接从第一个数据包开始就是加密的 HTTPS 流量，每台 proxy 都配有效的 TLS 证书，在线路上跟一般网站浏览难以区分。
...
审查强度更高的环境要另外评估。防火长城会做主动探测（active probing，主动连向可疑的服务器测试它是不是代理），DPI 的辨识能力也比多数地区强，Signal Proxy 在这种环境不保证能用。手边需要有备援方案，可以看 [WebTunnel 桥接](../community/setup-tor-webtunnel.md)、[Tor Snowflake](./tor-snowflake.md)，以及 [VPN 的风险与选择](./vpn-guide.md) 里的混淆协议一节。

!!! warning "境内使用者请先评估法律风险"

    中国对个人绕行审查长期属法律灰色地带，2025 年底官方公开示警绕行审查会被究责[^china]。使用 Signal 本身、以及连接一台境外转接服务器，在当地的处境跟使用 VPN 类似。这一节不评价任何人的选择，判断值不值得是当事人的事，相关的风险层次见 [在中国大陆的公开平台传播信息](../scenarios/mainland-speech.md)。
...
- **proxy 不隐藏「你在用 Signal」这件事**。对你的网络供应商而言，流量看起来是连到某个普通网站，不过使用 Signal 本身在当地已构成风险时，proxy 解决不了这个问题。需要连使用行为都藏起来，要看 [Tor Browser 进阶设定](./tor-browser-advanced.md) 或移动设备上的 Orbot 这类方案。
...
!!! warning "规划中，尚未上线"
    ...
    社区正在评估架设一台公开的 Signal Proxy。服务上线后，这一节会补上分享链接、取得地址的管道，以及更换地址时的通知方式。在此之前这页只是说明文件，没有可用的地址可以拿。
...
地址暴露得越广，被封锁得越快。实务上的做法是公开讲「我有一台 Signal proxy」，地址走私信、加密邮件或小群组给出去。同一台服务器如果服务对象太集中在一个地区，也会加速被当地审查系统盯上。
...
在台湾这类没有审查命令的地区，目前没有限制个人或组织提供这类转接服务的规定。实际要留意的是主机商的服务条款，以及带宽用量是否超出方案限制。人在对绕行审查工具有管制的司法管辖区，架设本身要另外评估，不能照搬。
...
??? question "proxy 运营者会被要求交出用户数据吗？"
    ...
    服务器上能留下的是连接 IP 与时间，消息内容与通讯对象都在端对端加密内，技术上拿不到。降低风险的做法是不开额外的连接记录、不长期保存 log。
