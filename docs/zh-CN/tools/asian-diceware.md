...
选字有两个刻意的限制。第一，外来语一律收已经进英文字典、写成一个单字的词，不收 `feng shui`、`kung fu`、`dim sum` 这类中间有空格的词组。第二，我们不自己音译华语，因为华语圈用过汉语拼音、威妥玛、通用拼音几套标准，香港等地另有粤语拼音（Jyutping）等系统，拼法会打架，所以只收英文拼法已被字典固定下来的字。

亚洲外来语只占约 3.8%，一句六字密语大约每五组才会出现一组带亚洲字，其余都是常见英文字。这是刻意的设计，安全性与 EFF 原版完全一致，亚洲味只是多一点亲切感，不必期待每次都抽到 `tofu`、`oolong`。

??? note "为什么亚洲外来语只占约 3.8%"
    像啤酒的酒精浓度（ABV）一样，3.8% 是刻意挑过的比例（7776 字中 292 个）。上限取决于英语实际吸收成「单一字典词」的可辨识亚洲外来语有多少。把 OED、Merriam-Webster、Cambridge 查过一轮，华语圈读者认得出来的大约 330 个，其中 292 个纳入清单，另外约 40 个留作备援，可辨识的外来语几乎就到顶了。
...
**方法一，用实体骰子查表：**掷 5 颗骰，由左到右读成一个五位数（每颗 1 到 6）。那张表是照代码从小到大排列的，翻到、或用搜索找到开头是你这五位数的那一行，后面接的字就是抽到的字。重复 6 次，把 6 个字用 `-` 接起来。例如掷出 `6 3 4 4 4`，读成 `63444`，查到 `tofu`。六次掷完就得到一句像 `tofu-oolong-gecko-mango-bazaar-haiku` 的密语，读得出来、抄得下来，比一串乱码好记得多。
...
- **和信任的人约定暗号：**把词表当成只有你们懂的暗号本，用来确认身分或传递约定好的信号，展开见下方〈和信任的人约定一组暗号本〉。
...
- **一定用真随机数：**实体骰子最简单直接。若改用程序产生，请走密码学安全的方法（见上面〈给会写程序的人〉），不要自己挑字，也不要挑「看起来很随机」的字。人挑的字猜起来容易得多。
...
想提升日常密码的强度，读到上一节就够了。这节是这份词表的进阶用法，记者、公民团体、需要低调传递信号的人可能特别用得上。
