...
??? note "为什么亚洲外来语只占约 3.8%"
    ...
    - **拿冷僻词充数**（`puttee`、`howdah`、`nilgai`、`maund`），多数人拼不出、念不准、也记不住。这会破坏清单要守住的 EFF 特性，也就是抄得下来、念得回去、不容易出错。
...
词表有两种大小，正式用途一律用默认的 `7776`（每字掷 5 颗骰）。另一份 `1296` 是它的子集、只要 4 颗骰，清单较短、适合测试、示范、教学。两份的骰子代码不同，用哪一份就查哪一份的表，不要拿错。
...
想提升日常密码的强度，读到上一节就够了。这节是词表的进阶用法，记者、公民团体、需要低调传递信号的人可能特别用得上。
