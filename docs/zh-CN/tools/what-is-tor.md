...
??? question "Tor 真的不会被破解吗？"
    ...
    没有绝对安全。已知攻击面包括：同时控制入口与出口的时序关联分析（学界研究、极大规模才可能）、浏览器指纹（Tor Browser 已强化但不是零）、使用者操作失误（在 Tor 里登入带身分的账号就破功）。对绝大多数威胁模型，Tor 仍然是匿名工具里的当前最佳实践。要看更深的攻击面讨论可以从 [威胁模型如何建立](../basics/threat-model.md) 开始。
