...
[Tor（The Onion Router）](https://www.torproject.org/){target="_blank"} 是由志工营运、透过多层加密与随机路径把网路连线匿名化的开源网路。Tor 解决的问题很具体：连上一个网站时，默认会把 IP 位址、浏览指纹、连线时序留给对方与沿路所有观察者。Tor 把这条连线拆成三段，让没有任何单一节点同时知道「你是谁」与「你在连什么」。
...
中继点清单是公开的，这是 Tor 设计的一部分：任何人都能验证网路上有哪些节点、谁在运营、运营多久了。公开可查也是 Tor 比起私有匿名网路（例如某些 VPN）多一层信任的原因。
...
??? question "ISP 看得到我在用 Tor 吗？"
    ...
    在不开桥接的情况下，看得到。ISP 看到你的 IP 连到一个公开 Tor 入口节点的 IP（公开清单查得到），它会知道你在用 Tor，但看不到你接下来连什么。这层「技术可见度」在哪里都一样，但「被看到在用 Tor」会不会有后果，要看所在地区的威胁模型。以香港为例，2020 年《国安法》后监控与寒蝉效应升高，记者、社运工作者被观察到使用 Tor 虽不违法，仍可能引来额外关注，判断方式见 [威胁模型如何建立](../basics/threat-model.md) 与 [VPN 的风险与选择](./vpn-guide.md)。如果你在意这层暴露（例如在敏感工作场所网路，或身处监控较强的地区），可以开桥接 + Pluggable Transports 把这层也藏起来。

??? question "Tor 速度为什么这么慢？"

    三层加密 + 三跳节点 + 出口节点频宽有限是物理结构，不会大改善。但有两个简单的调整能改善体感：在 Tor Browser 设定里换出口节点地区（避开壅塞国家）、避免高频宽内容（看 8K 影片不是 Tor 的设计目的）。整体的 Tor 网路容量取决于全球有多少志工愿意运作节点，[Tor Relay 校园建立](../community/relay-on-campus.md) 等社群推动也是为了把更多本地频宽接进网路。
