...
前面三家的分野（消费版可能训练、企业版默认不训练）是境外服务的通则。DeepSeek、豆包、Kimi、通义等在境内提供的生成式 AI 服务适用另一套规则，前面的判断方式不能直接套用。

《生成式人工智能服务管理暂行办法》2023 年 8 月 15 日施行，由网信办会同六个部门发布[^cn-genai]，跟工作使用直接相关的规定有：
