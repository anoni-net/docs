...
想匿名浏览、传档、或贡献网络自由基础建设的人，从这群开始。其中 Signal Proxy 与 VPN 两篇不属于 Tor 家族，同样是绕行封锁会用到的连接工具，放这里对照着读。
...
- [什么是 Tails](./what-is-tails.md)：从 USB 启动、默认走 Tor、关机后不留痕迹的可携式系统。
...
想把「连不上」、「跑很慢」变成可引用的公开数据，或想做封锁测量活动的人，从这群开始。
