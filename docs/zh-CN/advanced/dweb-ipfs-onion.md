...
- **自架服务器**：自己有 IP、自己有 DNS。完全可控，也完全集中。被封 IP、被没收服务器就无法连接。
...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/ipfs-vs-onion-v2.zh-TW.svg" alt="IPFS 与 Onion 架构对照。IPFS 走 gateway → DHT → peer，主要解决抗删除。Onion 走 Guard → Middle → Rendezvous → HSDir → 服务，主要解决连线匿名">
</figure>
...
- IPFS 镜像：固定 CID 对应每次发布的内容，社群志工可协助 pin（做法见 [帮忙 pin 文件站的 IPFS 镜像](../community/pin-ipfs-mirror.md)）。
...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/anoni-deployment-v2.zh-TW.svg" alt="anoni.net 文件站三轨部署：同一份 GitHub repo 透过三套 build script 处理路径差异，分别部署到主站 anoni.net、IPFS 网络、.onion 隐藏服务">
</figure>
