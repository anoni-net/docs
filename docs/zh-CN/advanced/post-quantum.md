...
- **数据的长期价值高**：政府机密、医疗记录、商业情报、长期身份数据（社会安全号、护照）。
...
<figure markdown="span">
    <img class="brand-frame" src="https://assets.anoni.net/diagrams/pq-timeline.zh-TW.drawio.svg" alt="后量子密码迁移时程：从 2023 Signal PQXDH、2024 NIST FIPS 与主流浏览器启用、到 2030–2035 主要系统应完成转换、2040 旧密码学硬时间表">
</figure>
...
- OpenSSH 9.0（2022）加入 sntrup761x25519 hybrid，9.9（2024）支持 ML-KEM hybrid。
...
- **VPN 与 IPSec**：商用 VPN 对 PQC 的支持不一致，公司网络规划时要纳入评估。
- **长期文件加密**：法务文件、医疗记录、研究数据需要评估「这份文件十年后仍要保密吗」，是的话要选 PQC 加密。
- **硬件安全模组**（HSM）：金融、医疗领域的 HSM 需要供应商提供 PQC 算法支持，更新周期长。

## 在地脉络：台湾的政府、金融、医疗合规时程
...
- **医疗**：医疗影像与电子病历需长期保存（台湾《医疗法》规定病历至少保存 7 年，部分影像与追踪数据更久），对 HNDL 风险高，可能是国内最早需要主动规划 PQC 的领域。

这节列的主管机关与合规时程都是台湾特定的。香港的对应机关是香港金融管理局（HKMA）与证监会（SFC），病历等长期数据的保存规范也不同。香港目前尚未查到公开的 PQC 迁移时程。跨地区通用的判断仍回到 NIST 与 ENISA 的 PQC 迁移时程，先处理 HNDL 风险高的长期数据。

对个人与一般组织，现在不需要急着手动部署 PQC。优先动作是：

- 采用会自动更新到 PQC 的工具（Chrome、Firefox、Signal、Cloudflare）。
- 评估自家的「长期保密数据」清单，把 HNDL 风险高的数据优先处理。
