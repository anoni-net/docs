...
实作上，现代协议普遍使用基于椭圆曲线的 X25519，比早期的有限体版本更小、更快，且抗侧信道攻击。Signal、Tor、TLS 1.3、SSH 默认都是 X25519。
...
前向保密（Forward Secrecy，FS）的目标是：即使长期密钥将来被攻击者拿到，过去已经传出去、被攻击者录下的密文，仍然无法解密。

做法是每次连线（或每段时间）都用一把临时密钥（ephemeral key）交换出当下的工作密钥，工作密钥用完即丢。攻击者拿到的长期密钥只能验证身份（这个人是不是真的 Alice），不能还原过去的工作密钥。
...
Double Ratchet 是 Signal Protocol 的核心，名称来自两种同时运作的密钥更新机制（两种「棘轮」）：
...
2. **后向保密**（Post-Compromise Security，PCS）：即使装置一度被入侵、密钥被窃，只要在下一次 DH 交换时攻击者没有拦下消息，安全状态就会自我恢复。
...
Double Ratchet 在 2014 年被 Open Whisper Systems（后来的 Signal Foundation）整合进 Signal Protocol，后续被 WhatsApp、Facebook Messenger Secret Conversations、Skype Private Conversations 采用。

## 群组通讯的两条路：Sender Keys 与 MLS

一对一通讯的解法成熟了，群组的成本却完全不同。100 人的群组、每则消息要不要做 100 次 DH？业界目前有两条主流路线。
...
MLS 用树状密钥结构（TreeKEM）让加入、退出、消息加密的成本都压在 O(log n)，理论上可支援上万人群组仍维持 PCS。设计目标是把 E2EE 从「两三百人就吃力」推到「企业级群组」。

2023 年 IETF 正式发布 RFC 9420。Webex、Cisco、Discord 已采用或宣布采用，Signal 自己 2024 年开始实验 MLS-based 1:1 与群组设计。

两条路的取舍：Sender Keys 简单、实作成熟、适合 Signal 规模。MLS 复杂、规模上更乾净、适合需要管理大型工作群组的场景。

## 多装置同步的取舍

E2EE 一个常被忽略的痛点是「多装置」。手机、平板、笔电要同时看到同一段对话，密钥怎么办？
...
Signal 在 2024 年加入加密消息备份，采取「用户输入长 passphrase + 服务商不知道」的设计，试图在便利与隐私之间找平衡点。
...
| **Session** | Onion routing + 对等加密 | 半中心化 | 24 字 mnemonic 跨装置 | 走 Tor-like 网络降低 metadata | ✅ |
...
- **Session**：流量走类 Tor 网络降低 metadata，跨装置只靠一段 mnemonic 门槛低。代价是消息延迟较高、无前向保密的旧版设计仍在转换中。

## 在地脉络：台湾社群为何用 Signal 与 Matrix

台湾最普及的消息工具 Line 默认不是 E2EE。Letter Sealing 功能（2015 年推出）只在双方都启用、且使用最新版时才生效，群组消息不在范围内。对话内容仍经过 Line 服务器，服务器端可在司法协助请求下提供内容。

anoni.net 社群与多数关注隐私的台湾用户偏好两条路：
