...
社运场景里，伤害你的数位风险多半来自群组通讯、装置被检查、现场照片与位置信息的暴露，跟个人匿名是两回事。这些风险跟你身边还有谁、组织内部记录留在哪、现场有没有人拍到你都有关。这篇文章把一场行动拆成四个阶段来谈：动员前的准备、行动现场、行动之后、跨组织协作。每个阶段配对应的工具与设定，并对照台湾与香港两地行动圈累积的实务。
...
每个场景的对手不一样，但都围绕同一个核心问题。你现在的数位足迹里，哪些东西被取得后会伤害你或同伴。这篇文章把问题拆成可以实际调整的具体设定。
...
3. SIM 卡用预付卡。要注意多数地区的预付卡同样要实名登记，能做的是不要登记在跟主号码同一个人名下。跨境情境的取舍见 [出差与研讨会的数位准备](./asia-travel.md) 的干净机一节
4. 平时关机，行动当天才开机
...
- **旧文章的留言**：你 5 年前的留言可能透露住处、学校、家人关系，PTT 的搜寻功能会被别人拿来翻旧帐

审计步骤：

1. 用自己的姓名、惯用 ID、email 在 Google 图片搜寻
2. 看出现的内容是不是你想被看到的
3. 各平台「下载我的资料」汇出一份备份，再删掉公开可见的旧内容

数位足迹审计不只服务这次动员，更是长期的数位卫生。每次行动前抽 30 分钟检查，会比事后处理轻松很多。
...
- **紧急锁定快捷键**：iPhone 连按 5 下电源键（需先在设定的紧急 SOS 开启对应选项）会切到 PIN-only 模式，停用生物辨识直到下次解锁。Android 各厂路径不同，多半是长按电源键或同时按电源加音量键叫出「SOS 紧急」或锁定选项，可先在系统设定搜寻「锁定」、「紧急」找到自己机型的做法。练到不看也能按
...
现场通讯主流仍是 Signal、Matrix，两者都走行动数据。**离线 mesh**（手机之间直接用蓝牙、Wi-Fi 互相接力传讯，不经过基站或网路）工具（Bridgefy、Briar）在实务上有限制，要先理解才不会误用：
...
国内的法律支援管道（民间司法改革基金会、台湾人权促进会、法律扶助基金会）见前面「取得法律支援的管道」。另外：

- 国际数位安全：[Access Now Helpline](https://www.accessnow.org/help/){target="_blank"}（24/7、多语）
