---
title: 记者保护消息来源
description: 从第一次接触、引用层级与知情同意、文件交换到多来源区隔与报导刊出后的数字足迹处理，含台湾、香港与中国大陆的脉络。
icon: material/newspaper-variant-outline
---

# :material-newspaper-variant-outline: 记者保护消息来源

调查报导的风险，最重的一端往往落在消息来源，而不是记者。记者就算被盯上，背后通常还有编辑部、媒体律师、业界同行的支持。消息来源就不一定，可能是公司内部的中阶员工、政府机关的承办、被监控的社运参与者，他们暴露的代价远比记者高。

这篇文章从一个常见流程出发：第一次接触的安全管道、确认对方身份、谈定引用层级与知情同意、敏感文件交换、访谈记录保存、多个来源之间的区隔、报导刊出后双方都需要做的数字整理。另外处理两种常见但少被写到的情况：来源不熟悉数字工具，以及采访涉及中国大陆。每一步都对应到具体的工具与威胁模型，并引用台湾与东南亚调查报导圈常见的实务做法。
...
- **邮件别名**：门槛最低的入口，公开一个转发别名，对方用任何信箱都寄得进来。防护力也最低，转发商看得到对方的原始发件地址与来源 IP，所有别名又指向你同一个信箱，敏感度一升高就要引导对方换到上面几种，取舍见 [邮件别名怎么用，以及它把信任交给谁](../tools/email-alias.md)
...
- 对方愿意视频（Jitsi、Element Call）但不录影，看到本人能再排除一部分风险
...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/source-attribution-levels-v2.zh-CN.svg"
        alt="四层由深到浅的横条，代表记者与消息来源可以约定的四种引用层级，越往下来源的暴露面越小。第一层是具名，意思是可以写出姓名与职称，报道里会出现姓名、职称与原话。第二层是匿名引用，意思是可以引述内容但用角色代称，报道里会出现像「一位参与该项目的工程师表示」这样的句子。第三层是背景，只用来理解事情与指引查证方向，来源不出现也不被引述。第四层是不使用，内容完全不进入报道。下方的红框提醒匿名引用最容易出事，因为代称本身可能就足以指认，一家公司里符合描述的只有三个人时，代称等于点名了三个人。">
    <figcaption>逐段确认，不要整场谈完之后才一次确认。不要承诺做不到的事。可以承诺的是具体行为：不交出记录、不在编辑部以外提到他、报道刊出前让他看过涉及他的段落。</figcaption>
</figure>
...
- **发稿前撤回**：尊重，把该来源的内容改用其他来源或公开数据佐证，不能佐证的部分拿掉
...
多数数字安全指引默认对方能自己装 Signal、清 metadata、开 Tor Browser。实际上高龄、视听功能退化、没有智能手机使用习惯的来源不少。硬要对方学新工具，结果通常是装错、设错，或者直接放弃联系。

原则是把技术负担移到记者这一侧。

### 先判断对方能做到什么

- 对方现在用什么联系，市话、短信、微信，还是要靠家人代为转达
...
- 市话与短信会留下通联记录，但对方真的能用。对手只到公司内部调查这一层时，取舍可以接受。要记得两件事：案件随时可能升级到能调取通联记录的层级，而通联记录的保存期限通常比案件周期长
- 需要更高保护时，改成由记者到现场，或者透过一位对方信任、技术能力较好的中间人
- 不要为了安全把对方推到他无法使用的工具上。无法使用的工具等于没有工具，对方会退回最方便的管道，那通常最不安全
...
年纪大的来源不一定清楚「上网」跟「上报」的差别，也不一定预期报导会被搜索引擎长期保存、被家人看到。同意的内容要具体说明：报导会公开在网络上、可能被转载、家人与邻居可能看到。必要时请对方复述一次他理解的内容。
...
录音的话，用独立录音装置（[Tascam](https://tascam.com/){target="_blank"} 或 [Zoom](https://zoomcorp.com/){target="_blank"} 的手持式录音机，这里的 Zoom 是日本录音设备厂商 Zoom Corporation，与视频会议的 Zoom 无关），不要用手机。手机不适合的核心原因有三个：
...
- **独立录音机可以单独销毁**。SD 卡是物理上可销毁的单位，不会牵连其他数据，手机要销毁录音却得连带处理整台或整个分区。长访谈（2 至 3 小时）手机也常因来电、通知、低电量中断，独立录音机更稳。
...
- 报导引述前再对照原档确认用词，但成稿后尽可能销毁识别关联（例如代号对应表单独保存）
...
- 只有少数人知道的事，换个问法从公开数据或制度面切入，不要直接抛出来
...
- 人数少的时候改写描述、模糊时间点，或者找公开数据佐证同一件事
- 多个来源的内容混写成一段，比逐段标明来源安全

## 报导刊出后的整理

报导刊出常常是消息来源风险升高的时刻。对方公司会启动内部调查、检视员工访问记录、抓出可能的泄漏者。记者这边要做的：
...
报导本身的公开数据、引用、查证记录要好好留存（这是面对诉讼的依据）。但跟消息来源往返的私下对话，原则上越早销毁越好。具体分类：
...
前面几节处理的是保护消息来源。这一节处理另一种情况，记者本人被当成情报目标。

2026 年 9 月 Anthropic 的滥用检测报告记录了两种与记者直接相关的手法。一是侧写与反制档案，报告第 78 页写，一个与阿联酋相关的行动对 18 名欧洲议会议员与知名记者做了彻底的研究与侧写，并对批评过阿联酋在苏丹作为的联合国特别报告员汇整反制档案。二是账号被复制，第 70 页写，一个与伊朗反对运动相关的行动指派共用的 AI 代理复制一名真实运动者的 Telegram 账号，用波斯语与他的联络人即时对话，报告写就他们所知，那些联络人并不知道自己在跟一个 AI 辅助的账号说话。逐段的页码对照见 [Anthropic 九月威胁报告写到台湾的四个段落](../blog/posts/2026-anthropic-threat-report-taiwan.md)。

经常与国际人权机制往来、也常接受采访的人，手法的重叠度最高。

### 事前约好一个验证方式

被冒充的时候，能保护你的是身边的人怎么辨识。这件事要在事前做：

- 跟固定往来的来源、编辑与同业约好一个验证方式，共用的验证词，或者换一个管道回拨
- 换联络管道时，用旧管道宣告一次，不要只从新管道单方面出现
- 对方的语气、要求或节奏突然改变时先停下来确认，尤其是索取新文件、要求改用新平台、催你立刻回复这三种
- [网络中断时的准备与应对](./shutdown.md) 里跟联络人约定会合与验证方式用的是同一套逻辑，两边可以共用同一组约定

### 定期看自己的名字被拿去做什么

- 每季用自己的姓名、惯用 ID 与服务的媒体名称搜索一次，包含各社群平台的账号搜索
- 发现被复制的账号时，先通知常用联络人，再向平台举报。举报之前把截图与网址留下来
- 被冒充影响到的是你的联络人，通知他们的优先顺序高于平台处理的速度

### 跨国协作的私人群组

跨境调查经常靠私人群组维持，报告第 83 页引述的商业监控厂商服务内容，第一步就是用假账号混进私人群组再收割成员清单。成员验证的具体作法见 [社运行动者的数字准备](./activist.md) 的跨国协作群组的成员验证一节，记者这一端额外要注意的是，群组里不要放来源的代号对应表与完整联络名单。

### 被建档跟被处置是两件事

记者本人被写进档案，下一步通常是列入名单与持续追踪。实际会不会受到影响，跟本人或所属媒体会不会进入对方辖区的关联更大。这一层的能力边界见 [监控现在做得到什么](../basics/surveillance-capability.md) 的公开来源的情报建档一节。
...
- 部分台湾媒体日益重视记者与消息来源的数字安全（报导者长期有资讯安全相关报导），但是否设有专责的数字安全编制或外部顾问，各家不一
- 跨境议题（中国、东南亚）通常会跟国际组织合作（OCCRP、Forbidden Stories），他们有自己的安全标准

### 台湾的求助渠道

- 报导者基金会数字安全资源：[https://www.twreporter.org/](https://www.twreporter.org/){target="_blank"}
...
- 境内通讯默认可被调取。微信没有端对端加密，Signal、WhatsApp、Telegram 在境内被封锁，受访者需连上 VPN 才能使用，而个人翻墙在境内属违法灰色地带
...
- 带乾淨机入境，敏感数据留在境外，需要时透过加密连线取用。逐项做法见 [出差与研讨会的数字准备](./asia-travel.md) 的中国大陆段
- 境内不要处理来源身份相关的文件
...
- **[Access Now Helpline](https://www.accessnow.org/help/){target="_blank"}**：24/7 多语的数字安全求助
