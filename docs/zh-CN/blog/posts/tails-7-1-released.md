...
动态语系切换现在运作正常，可以根据系统设定自动调整语言，而不是需要手动配置或重新启动。对于在多语言环境中操作的用户，这减少了切换身份的困扰。虽然语系泄漏仍然是一个进行指纹识别的途径，但强迫用户在语言变更时重新启动，会降低操作安全，因为这可能让他们跳过保护。
...
Tails 7.1 的用户从版本 7.0 自动升级时，会保留持久储存区。不过，若手动从 USB 或 ISO 重新安装，则会删除现有的数据。该项目提供了针对 Windows、macOS 和 Linux 的安装指南，但也警告用户，如果选择重新安装而非升级，将会摧毁持久储存，这依然是让用户混淆的一个持续问题，因为他们可能会不小心删除储存的数据。
