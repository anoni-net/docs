...
[出差与研讨会的数位准备](../../scenarios/asia-travel.md)在八月初做过一次全页查证，六个方向独立核对来源，结果撤回三处查不到实据的叙述。
...
另外两处是跨页之间的冲突，[社运行动者的数位准备](../../scenarios/activist.md)的「预付卡购买时不要用实名身分证办」与出差页新增的十四地实名对照直接矛盾，而新章节还两次把读者导向出差页。[记者保护消息来源](../../scenarios/journalist.md)把市话与简讯的取舍写得像许可，补上案件随时可能升级到能调阅通联记录的层级，而通联记录的保存期限通常比案件周期长。
...
能重算的原因是栏位统计终于补上。[OONI 测量资料结构导览](../../community/ooni-data-format.md)上线时就写着，最小的一步是在逐行解析时多取 `test_keys.blocking`，统计就能从测量笔数延伸到各 ASN（自治系统编号，网络上一个业者或组织所管的一群 IP）的异常分布。程序一直没有做，是文档与程序之间唯一一处只写在纸上的规划，八月初补上之后，新数据一产出就推翻了上面那段叙述。
...
前面几节的错误有一个共同的上游，规范写了但没有工具挡着，内容就会持续偏离，直到有人读到为止。[贡献者百科](../../community/contributor-handbook.md)写着口语词改书面语，而 `docs_style_lint.py` 只针对其中一个字实作了规则。全站扫描出 605 处那个字以外的口语词，网站自己的工具对现况回报 `0 error`、`0 warn`。
