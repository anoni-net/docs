...
每笔测量最后会被标成 verified、unverified 或 failed 三种状态，数据用户可以自己决定要采信到什么程度。这套系统上线后，有些效果要长期观察才看得出来。OONI 会持续追踪 verified、unverified、failed 三类测量的比例变化，用来诊断是不是某次客户端更新让协定实作出现错误、或老探针没更新。他们也会统计各协定版本的探针数量，以及在服务器端执行一笔验证要花多少时间，及早抓出性能退化。
