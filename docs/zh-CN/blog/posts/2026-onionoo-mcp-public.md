...
匿名网络社区自架的 `onionoo-fastapi` 服务以 v1.0.0 对外公开，站点位于 <https://onionoo.anoni.net>。它把 Tor Project 官方的 [Onionoo](https://metrics.torproject.org/onionoo.html){target="_blank"} API 包成两种接口，一份是补上 OpenAPI 规范的语义化 HTTP API，一个是 Model Context Protocol（MCP）server。
...
存档、重启客户端，工具列表中就会出现 onionoo 这组工具，可以直接用自然语言要求代理查询。本机 stdio transport 安装方式、完整工具一览、权限调整、自架（Docker）等细节，全部收在完整使用文件。
