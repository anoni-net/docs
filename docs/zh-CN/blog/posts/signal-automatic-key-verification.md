...
!!! note "以下为 anoni.net 社群补充，非 Signal 原文内容"
    ...
    原文的比喻写得完整，技术脉络与取舍留在文外。以下九则补充，加上一组常见问题与延伸阅读，给想再往下读的人。
...
2025 年 2 月多份威胁情报报告指出，针对乌克兰军方人员与记者的攻击者，把设备关联用的 QR code 伪装成群组邀请、安全警告与前线单位的应用程序页面，诱导目标把攻击者的设备连上自己的 Signal 账号，整个过程不需要任何密码学上的突破。

对应的操作是定期打开设置里的「已关联设备」，看到不认得的项目就解除关联。密钥透明度不会替你看这一项。
...
可验证随机函数让服务器用一把只有自己知道的密钥，把标识符映射到日志里一个看起来随机的位置，同时附上一份任何人都能检查的证明，说明映射确实照规则生成。外人少了那把密钥就无法自行还原名单，审计方仍然验证得了日志的一致性，查询者也只有在向服务器查过某个标识符之后，才会知道它落在哪个位置。
...
出入境查验或设备离开视线一段时间之后，值得做的是检查已关联设备清单，以及重要联系人的安全码有没有变动。自动密钥验证对这两件事都帮不上忙。

在 Signal 被封锁的地区，功能仍然依赖连得上 Signal 的服务器。连线走代理或审查规避工具时，密钥透明度的查询会一起经过那条通道。原文提到查询不带身份验证，日志看不出是谁在查，通道的运营者仍然看得到你在跟 Signal 通讯。

原文提到你手上留着联系人的旧号码时，自动密钥验证会直接无法使用，跨境移动、经常换号的社群遇到的频率会比一般用户高。退路仍然是安全码，当面扫 QR code 最快，只能通过语音核对那串数字时，前提是你认得对方的声音。
