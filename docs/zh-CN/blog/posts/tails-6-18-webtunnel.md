...
summary: "Tails 支持 WebTunnel 网桥类型，帮助更多无法直接连入 Tor 洋葱路由的人，透过这种方式连入"
description: "Tails 支持 WebTunnel 网桥类型，帮助更多无法直接连入 Tor 洋葱路由的人，透过这种方式连入"
---
...
!!! note "翻译说明"
    ...
    我们目前还没有针对“**Bridge 桥接类型**”做专门介绍，桥接节点主要是协助无法直接连接到 Tor 的洋葱路由中，通过桥接节点接入。目前有一种最快的方式建立桥接点，是通过伪装成**在线视频**的连接方式来进行转接，仅使用**浏览器**就可以帮助建立。详细信息或试用可以参考「[Tor Snowflake 桥接点建立（网页版）](../../tools/tor-snowflake.md){target="_blank"}」。
