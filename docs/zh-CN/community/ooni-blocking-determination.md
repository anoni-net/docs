...
[OONI 测量数据结构导览](./ooni-data-format.md) 说明各栏位的位置，本页说明判定栏位的计算方式。`blocking: "dns"` 在多数情况下并不足以断定某个网站在台湾被封锁。
...
<figure markdown="span">
    <img src="https://assets.anoni.net/diagrams/ooni-blocking-decision.zh-CN.svg"
...
上面那张表列的是「差异出现在哪里」对应到哪个判定，画成流程之后会多看到一件事：判定是有顺序的。DNS 阶段一出问题就不再往下比对，所以后面那几个 `*_match` 栏位在前三种情况下全是 `null`。下一节的五笔实际测量，就是这四条路径各自的例子。

## 四种判定对应的证据

后续章节引用的五笔测量都是 2026-08-04 的公开数据，可在 [OONI Explorer](https://explorer.ooni.org/){target="_blank"} 查到原始内容。
...
台湾的数据中有 `http-diff`，但样态与封锁告示页不同（见文末台湾现况），以下改用印尼的测量说明典型的告示页样态。
...
`blocking` 有值而实际上没有网络干预，是数据集中的常态。前述 `tkec.com.tw` 的 `80` 埠不通即为一例，以下是一笔更隐蔽的。
...
OONI 在数据集层级亦处理同一类问题，作法见 [OONI 如何分辨坏掉的量测数据](../blog/posts/2026-ooni-faulty-measurements.md)，该文整理了 OONI 过滤异常投稿所用的启发式规则。

## 从单笔测量到可信结论

将观测推进到「某个网站在某地被封锁」，单笔数据并不足够。实务上须补上几个维度：
...
4. **`confirmed` 栏位**：OONI 后端以已知的封锁告示页指纹比对测量，命中时将 `confirmed` 标为 `true`。该栏位位于 [measurements API](https://api.ooni.org/api/v1/measurements){target="_blank"} 的回应中，属于后端分析结果，不在 Probe 产生的原始数据内。

跨时间与跨 ASN 比对的批次取用作法，见 [ASN 观测数据提取与分析](./asn-coverage-howto.md)。
...
以下数字取自 2026-08-05 往前 24 小时的台湾 `web_connectivity` 全量数据，共 22,105 笔测量。统计方式是把该区间 S3 上的每一笔逐行解析，取 `test_keys.blocking` 依 ASN 累计：
...
另外抽查 100 笔异常测量，`confirmed` 为 `true` 的有 0 笔，与 [ASN 自治网络观测数据分析](../taiwan/ooni-asn-coverage.md) 长期以来的描述一致。

!!! warning "小样本会给出错误的分布"

    本页初稿曾以 measurements API 异常清单的 40 笔抽样估计判定分布，得到 `dns` 最多、`http-diff` 为零的结论。改用全量统计后两点皆不成立：`tcp_ip` 为 `dns` 的两倍以上，`http-diff` 亦确实存在。API 的异常清单另有排序逻辑，迳自视为随机样本会失准。判定分布应以全量统计取得，作法见 [ASN 观测数据提取与分析](./asn-coverage-howto.md)。
...
以上是单一 24 小时区间的快照，反映当日样态，不足以作为长期趋势的结论。趋势分析须涵盖更长时间，而台湾的观测本身即存在 ASN 集中度过高的限制，细节见前述 [ASN 自治网络观测数据分析](../taiwan/ooni-asn-coverage.md)。

## 延伸阅读

跨时间、跨 ASN 的比对从提取指南开始。
...
判定算法的完整定义在上游 [ts-017-web-connectivity](https://github.com/ooni/spec/blob/master/nettests/ts-017-web-connectivity.md){target="_blank"}，失败字串的统一命名在 [df-007-errors](https://github.com/ooni/spec/blob/master/data-formats/df-007-errors.md){target="_blank"}。
