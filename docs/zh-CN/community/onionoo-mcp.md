...
[Onionoo](https://metrics.torproject.org/onionoo.html){target="_blank"} 是 Tor Project 官方维护的中继节点资料 API：任何人都能通过 HTTP 查询目前全球 Tor 中继节点（relay）与桥接（bridge）的详细信息，包括指纹、IP、所在国家与 ASN（自治系统编号）、运行旗标（Guard、Exit、HSDir 等）、带宽与在线时间的历史时间序列。Tor Project 自家的 [Relay Search](https://metrics.torproject.org/rs.html){target="_blank"} 与多数第三方观测站，底层资料都来自这个服务。
...
- **[Model Context Protocol（MCP）](https://modelcontextprotocol.io/){target="_blank"} 服务器**：让 Claude Desktop、Cursor、Claude Code 等支持 MCP 的客户端，可以直接用工具调用的方式查 Tor 中继节点，不需要写 HTTP 程序码。

服务本身**不存储**任何 Onionoo 资料，只负责转发与重新包装响应。上游资料来自 <https://onionoo.torproject.org>。
...
**Claude Desktop、Cursor 或其他支持 MCP 的客户端**：在配置档的 `mcpServers` 区块加上：
...
需要先安装 [`uv`](https://docs.astral.sh/uv/){target="_blank"}（macOS 上 `brew install uv`，Linux 上请参考官方文件）。

### 2. 当作 HTTP API（推荐给写程序直接调用）
...
默认监听 `8000` 端口，OpenAPI 文件在 `http://localhost:8000/docs`，MCP 端点在 `http://localhost:8000/mcp`。
...
| `CORS_ALLOWED_ORIGINS` | 允许跨来源请求的网域清单 | 空（默认关闭） |
...
| 工具 | 用途 |
|---|---|
| `find_relay(query)` | 自由文字查询，自动判断输入是 40 码指纹、AS 号、IP，或昵称子字串 |
...
<figure markdown="span">
    ...
    <capture>模型汇总后的最终报告：running 数、总带宽、consensus weight、ASN 分布一次到位。底层数值来自上游 Onionoo（资料随时间变动，这只是某一时点的 snapshot）。</capture>
</figure>
...
<figure markdown="span">
    ...
    <capture>代理一步一步说出要查什么、选哪个工具、结果回来后下一步要做什么。文字之间夹着实际的 <code>aggregate_countries</code>、<code>get_details</code> 工具调用。把背后的 MCP 互动完整摊开来，方便除错或微调 prompt。</capture>
</figure>
...
- 每个请求都有 `X-Request-ID`，可在 log 与响应 header 对应，方便除错。
