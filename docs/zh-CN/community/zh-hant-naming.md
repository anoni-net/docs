...
| `zh_Hant` | 中文(正体) |
| `zh_Hans` | 中文(简体) |
| `zh_TW` | 地区标签（台湾），不是过时，但语意是地区而非字系。新内容优先用字系标签 `zh_Hant`，需要指定地区时用 `zh-Hant-TW` |
| `zh_CN` | 地区标签（中国），同理新内容优先用字系标签 `zh_Hans` |
...
??? question "「正体」是不是政治用语？"
    ...
    「正体中文」这个称呼确实在 2000 年代以后在台湾政府文档、教育部用字里逐步被推广，背后有历史与政治因素。但本页主张的「为什么用正体」是对「命名隐含的对比框架」的观察，与政治立场无关。同样的论证在不涉及政治的字典学语境中也成立。
...
??? question "我在中国大陆使用简体中文，看到「正体」会不会觉得不舒服？"
    ...
    如果你在日常对话中习惯说「繁体」，照你的习惯说，没问题。我们社区的选择是在自己贡献的内容里写「正体」，那是我们的内容里的事，对你没有要求。
