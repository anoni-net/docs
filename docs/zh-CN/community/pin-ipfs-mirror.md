...
anoni.net 文件站除了主站，还有一份 IPFS 镜像，让文件在主站被封锁或下架时仍然读得到。IPFS 上的内容要有节点 pin 才会存活。2026 年 8 月查过网络上的提供者，只有社群自己那一台，整份镜像目前是单点。你多架设一个节点帮忙 pin，网络上就多一份完整副本，那台机器被停权或出事的时候文档还在。
...
!!! tip "要帮忙，你需要"
    ...
    第一次要抓的整份镜像约 240 MB，来源目前只有社群那一台，建议一开始就设好 [跟社群节点保持常连](#peering)，抓起来会顺很多。
...
pin 文件站只会用到 IPNS 名称。Peer ID 要到想让自己的节点跟社群节点保持固定连接时才用得上，做法见下面的 [跟社群节点保持常连](#peering)。
...
!!! info ""
    ...
    - 浏览器打开看：[https://ipfs.anoni.net/](https://ipfs.anoni.net/){target="_blank"}，社群自架的网关。旧网址 `anoni-net.ipns.dweb.link` 在 2026 年 9 月 30 日停止运营，见 [2026 年 9 月之后的 IPFS 维护变化](#ipfs-maintenance-2026)
...
peering 是单向设定，社群节点那端没有对应条目，连接的保活责任落在你这边。[kubo 官方文档](https://github.com/ipfs/kubo/blob/master/docs/config.md#peering){target="_blank"} 提醒过，单向 peering 会占用对方节点的连接资源，镜像数量变多之后负担会集中在同一台。这个顾虑成立的前提是帮忙 pin 的节点够多，而现在提供者只有一台，第一次那 240 MB 全靠它，所以建议第一次设定就打开。等 pin 的节点多起来、来源不再集中在同一台，再拿掉也不迟。
...
- **本页的浏览网址已经换掉**：旧的 `anoni-net.ipns.dweb.link` 属于 dweb.link，新的 [ipfs.anoni.net](https://ipfs.anoni.net/){target="_blank"} 由社群自己运营。两者读的是同一个 IPNS 名称，靠 DNSLink 记录对应。pin 脚本用的是 IPNS 名称，不受网关影响。
...
`auto` 展开后的节点停掉之后，新装的节点可能连不进 DHT。这时候把社群节点设成固定连接对象最直接，做法见上面的 [跟社群节点保持常连](#peering)，`Peering` 只要填 Peer ID，kubo 会自己向 DHT 查地址。

## 维护与注意事项

- **会自动跟上新版**：文件站换 CID 后，下一次定时任务就会 pin 新版、放掉旧版，你不用做任何事。
- **每次更新只抓差异**：CID 每次发布都会变，但脚本是先 pin 新版成功、才放掉旧版。pin 新版的当下旧版的区块还在你的节点上，IPFS 只去抓真正不一样的区块。第一次要抓整份，之后一般的内容更新通常只有几 MB。动到导航栏或版式的那几次例外，全站 HTML 会一起重写，差异接近一半的镜像大小。
- **硬盘不会一直变大**：脚本 unpin 旧版后会执行一次垃圾回收，只留最新版本占空间。一份完整镜像约 240 MB（2026 年 8 月实测）。
...
## 不想自架节点时改用 pinning service 代 pin
