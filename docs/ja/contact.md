---
title: 連絡先と最新情報
description: いますぐ参加できなくても、私たちの動きを追う方法があります。
icon: material/email-fast-outline
---

# :material-email-fast-outline: 連絡先と最新情報

プロジェクトの進捗や活動のお知らせは、押しつけがましくならない範囲でお届けする方針でいます。ニュースレターに登録していただくと、更新をまとめて受け取れます。

- 登録先： <https://form.anoni.net/s/w21855zpca072rvgp0s2govj>{target="_blank"}

公開の場では話しにくい内容がある場合は、下記のメール アドレスからプロジェクト メンバーに直接ご連絡ください。

### コミュニティでの議論とオンライン会議

継続的な議論やテーマごとの協働には **Matrix**（Web クライアントは [Element](https://matrix.anoni.net/){target="_blank"}、ホームサーバーは `im.anoni.net`）、[**Cryptpad** による暗号化された共同編集](https://cryptpad.anoni.net/){target="_blank"}、オンライン会議には [**Jitsi**](https://jitsi.goodmeet.asia/){target="_blank"} を使っています。アカウントの申請方法と参加の手順は[コミュニティ ページ](https://anoni.net/docs/en/community/){target="_blank"}（英語）をご覧ください。

### 日本語での連絡について

日本語でのお問い合わせも歓迎します。返信は英語または中国語になる場合がありますので、あらかじめご了承ください。日本語版の訳文に関するご指摘は、[GitHub の Issue](https://github.com/anoni-net/docs/issues){target="_blank"} に書いていただくと、修正の履歴が公開の形で残ります。

---

- Anonymity Networks Community <whisper@anoni.net>
- PGP: [B7DF84305C7911D90D59A66061F66CF36EE386D4](https://anoni.net/B7DF84305C7911D90D59A66061F66CF36EE386D4.asc)

    ??? quote "PGP 公開鍵（テキスト）"

        ```plain
        -----BEGIN PGP PUBLIC KEY BLOCK-----

        
        l740CBMA/AkWo4nSmejtm1CJqGuAv/dVD2SNU5QKbJglsX03MhgN
        =TJp2
        -----END PGP PUBLIC KEY BLOCK-----
        ```
