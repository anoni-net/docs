...
The first transfer right after extra connections open is clearly slower: with 12 connections, 100 MB took 9 seconds instead of 2. The Prepare option is there to find out why. "Open connections on connect" opens the extra connections as soon as pairing finishes, and "Open and warm up" also sends a short burst of data (1 MB per channel). Pick the option on both devices before pairing, then compare the first transfer. To try the next mode, pick it and press "Drop extra connections". Every extra connection between the two devices closes, and the next send or prepare opens a new set, with no reload and no new pairing. In the log, `rttMs` is the round trip time, and `localNet` and `remoteNet` give the class of each side's address (LAN, mDNS, Tailscale and so on), never the address itself.
