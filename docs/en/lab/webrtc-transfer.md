...
Step 5 has three transfer settings: chunk size, channels per connection, and parallel connections. The defaults are 64 KB, 1 channel and 8 connections. Change one at a time and leave the others at their defaults. Both sides log the settings used for each run, which is what lets the results show which setting made a difference.

Parallel connections open several connections between the same pair of devices. In a 2026-09-25 test between an iPhone 15 Pro and Chrome on a Mac, the iPhone sent 100 MB at 4.8 MB per second over 1 connection and about 16.5 MB per second over 3, almost in proportion to the count. More channels inside one connection and different chunk sizes made no difference, so the limit applies per connection. A second round the same day on home Wi-Fi 6 (5 GHz) showed each connection from the iPhone holding at about 5.5 to 6 MB per second, about 33 MB per second over 6, with the round trip time staying under 20 ms. The Mac sent at 53 to 55 MB per second from 3 connections on, which is the ceiling of that network. Going further, the iPhone sent at about 37 MB per second over 8 connections and about 41 over 12, with each connection getting less. Twelve was only about a tenth faster than eight while its first transfer was the slowest of all, so the default stops at 8.

The first transfer right after extra connections open is clearly slower: with 8 connections, 100 MB from the Mac to the iPhone took 9 to 13 seconds instead of 2. Testing ruled out simply leaving the connections open for a while, since connections left idle for 7 to 14 seconds were just as slow the first time. Data has to flow over the new connections first. Warming each connection with 1 MB cut the first transfer to 3.5 seconds, still short of the 2 seconds of the second. The Warm-up option is there to find how much is enough, at 1, 4 or 16 MB per connection, and it runs in the background as soon as pairing finishes. Pick the same amount on both devices before pairing, then compare the first transfer. To try another amount, pick it and press "Drop extra connections". Every extra connection between the two devices closes, and the next send or warm-up opens a new set, with no reload and no new pairing. In the log, `rttMs` is the round trip time, and `localNet` and `remoteNet` give the class of each side's address (LAN, mDNS, Tailscale and so on), never the address itself.
...
The log is kept in this tab and survives a reload. Closing the tab or pressing "Clear log" removes it. Press "Export log", save the JSON, and attach it to [issue #553](https://github.com/anoni-net/docs/issues/553){target="_blank"}. Export from every device, so that timings from the sending and the receiving side are all there.
