...
- **Email**: don't register a secondary-layer account with a work address or a shared family inbox. ProtonMail or Tuta are reasonable privacy-respecting providers. Mainstream free providers generally demand phone verification at signup, and accounts opened from the same device and IP may be linked by the provider regardless, so expect that if you use one for a secondary layer. When the number of layers outgrows the number of mailboxes you can maintain, email aliases grow several outward addresses from one mailbox instead; the methods and their costs are in [email aliases, and who you hand your trust to](../tools/email-alias.md).
