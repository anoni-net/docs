...
[Mullvad](https://mullvad.net/){target="_blank"} goes furthest on privacy by design. Signup needs no email — you just get a random account number. Payment accepts cash by mail, Monero, and Bitcoin, so you needn't tie your identity in. In September 2023 it completed a migration to RAM-only across all servers; its infrastructure and apps have been audited multiple times by Cure53, its web app independently audited by Assured, and the client is open source. The Swedish police search mentioned earlier is the proof of Mullvad's no-log policy in practice.
...
[IVPN](https://www.ivpn.net/){target="_blank"} is a long-established Gibraltar service: strict no-log, accepts Monero and cash, supports multi-hop, client open-sourced under GPLv3, and audited by Cure53 for several years running. It deliberately drops affiliate marketing and limited-time promotions to avoid letting sales tactics distort user judgment.
...
- It overlaps heavily with Guardian Project's Orbot, differing in the development team and the underlying engine (Orbot uses the older C-Tor, Tor VPN uses Arti); the project hasn't said which replaces which[^torvpn-orbot].
...
- **Criminalized:** unauthorized use or provision may break the law, e.g. Myanmar.
...
- **Hong Kong.** VPNs are legal, with no Great-Firewall-level blanket blocking, but it's a grey area. After the 2020 National Security Law, police used Article 43 to require ISPs to block specific sites via DNS tampering, and HKChronicles and Hong Kong Watch were among those blocked. The 2024 Safeguarding National Security Ordinance and a decryption obligation in force from March 2026 (during a national-security investigation under warrant, a person can be required to hand over device passwords, with up to one year's imprisonment for refusal) have sharply tightened the legal basis for device searches and surveillance. Legal does not mean safe — journalists and human-rights workers should treat national-security monitoring and selective blocking as real risks[^hk].
- **Macau.** VPNs are legal with no systematic blocking; it's regulated. The Cybersecurity Law (Law 13/2019) requires real-name telecom registration, and ISPs must retain mapping records that can trace a connection back to a specific user for at least one year[^macau]. The connection itself is logged long-term, which sensitive tasks should account for.
