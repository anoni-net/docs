...
The zine on the table looks like this. Four A4 sheets folded together inside a wrapper, and both faces of the wrapper are covers, so either one can face out. The design write-up is in [the zine we print only for events](../blog/posts/2026-gg2026-printed-wrapper.md).
...
Passkeys are what we most want to talk about at the booth. If you support people in a region or a threat model where this design falls apart, we would rather hear it now than after we ship it.
...
Other subjects are just as welcome. We start out from Taiwan, and we want to meet more people working across the Asia-Pacific. Sinophone users are spread across Taiwan, Hong Kong, China, Japan and Southeast Asia, the situations they run into are connected, and the chances to compare notes are rare.

Most of us come from open-source and technology communities, and the day-to-day work is running services, writing teaching material, building browser-side tools and reading OONI measurement data. We would like to put that to use for people doing frontline work, whether that means judging if a tool actually holds against the adversary in front of you, breaking a procedure down into steps you can verify yourself, or going through a measurement dataset together. If something you are dealing with needs that kind of technical support, say so at the booth or leave a message in the Matrix room.
