...
## Adding the site to a home screen turns it into an app

This is something browsers have been able to do for years and almost nobody uses. Any website can be added to a phone's home screen or a computer's application list, after which it has its own icon and its own window, opens like any other app, and never touches an app store on the way.

For offline reading the difference is the entry point. When the network is gone, nobody thinks "open the browser, find the bookmark, which folder was it in"; they think "where is that icon". A minute of setup turns it into one tap.

- **Chrome, Edge or Firefox on Android**: open the site, then the browser menu, and choose Install app, Install, or Add to Home screen
- **Chrome or Edge on a computer**: an install icon appears at the right of the address bar, or choose Install anoni.net Docs from the menu
- **Safari on iPhone or iPad**: tap Share at the bottom, scroll down, and choose Add to Home Screen

Two things catch people the first time. On iPhone and iPad the app and Safari keep separate storage, so pages stored in Safari do not come along, and the app has to store its own copy the first time it opens. On Android, long-pressing the icon opens a shortcut menu with Offline reading in it, which jumps straight to the management page.

Once installed there is no separate update to manage: when the site ships a new version the same card shown above appears inside the app, and removing it works like removing any other app.

The cost belongs up front. Installing leaves an icon on the device, which is the opposite of the no-installation-trace point in the previous section. If a border crossing may involve an inspection, skip it: opening the site in a browser reaches the same offline content, at the price of finding the bookmark yourself. Tor Browser does not offer this, for the same reason it stores nothing offline.
