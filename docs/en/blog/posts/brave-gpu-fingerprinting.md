...
Tor reachability varies by jurisdiction and belongs in the same calculation. Mainland China blocks it heavily, so connecting there needs bridges or another entry method. Taiwan, Hong Kong, Macau, Singapore, and Malaysia generally allow a direct connection, and the constraint moves elsewhere, to what you publish and to the records that real-name SIM registration leaves behind. Hong Kong adds an obligation, in force since March 2026, to surrender a device password during a national security investigation, and it reaches transit passengers too. [Device minimization and border crossings in Asia](../../scenarios/asia-travel.md) has the jurisdiction-by-jurisdiction detail, and the [Regional Observatory](../../regional/index.md) covers the reachability measurements behind it. Brave's fingerprinting protection is a browser feature that touches none of this, and it works from installation.
