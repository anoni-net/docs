...
- **An email alias** is the lowest-barrier option: publish a forwarding alias rather than your main address and a source can write from any mailbox without learning a new tool. It is also the weakest, because the forwarder sees the sender's originating address and source IP, and every alias points at the same mailbox of yours. Move sources to one of the options above as soon as sensitivity rises; the trade-offs are in [email aliases, and who you hand your trust to](../tools/email-alias.md).
...
Technique does not protect a source who is identifiable from the story itself, so settle how the material will be used at first contact rather than at publication. Agree on the level per topic, not per conversation: on the record, quoted anonymously by role, background only, or not used at all. The anonymous-quote level is where sources get burned, because the role description can identify them on its own — if only three people fit "an engineer on that project," the attribution names all three. Widen the description or drop to background.
...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/source-attribution-levels-v2.en.svg"
...
In practice that means meeting in person where you can, at a place that costs the source little to reach; avoiding phone and video with a source whose hearing is poor, in a room quiet enough for a hearing aid to work; keeping sessions short; and leaving no notes, tool instructions, or business cards behind in their home. Where a channel is unavoidable, a landline or SMS the source can actually operate may be the right trade-off against an encrypted tool they cannot, since an unusable tool means they fall back to the least safe channel available. Treat that trade-off as provisional: a case can escalate to an adversary who can obtain call records, and those records typically outlive the story. Strip metadata from their files yourself rather than teaching them, remind them to delete originals from their own phone, and be careful about handling their device, since your actions leave traces that a later search will find. Consent also needs more care here: a source may not distinguish "online" from "in the paper," so state plainly that the story will be public, searchable, and visible to family.
...
### Keeping multiple sources apart

Once a story has more than one source, you carry a second duty: keeping them from learning about each other. If one is identified, the subject organization works outward through their circle to find the next; sources who know others are talking change their accounts; and a source who genuinely does not know who else spoke has nothing to give up under questioning.

Use a different channel per source, keep separate code-name schemes with the mappings in separate files rather than one master table, and split interview notes by source. The riskiest step is verifying one source's detail with another: put a specific claim to a second source and they now know someone talked, and can usually guess who. Abstract the detail, approach it through public or procedural framing, and treat it as independent corroboration only when the second source volunteers it. The same problem returns at the writing stage — a fact only a handful of people could know is an attribution in disguise, so count how many people could plausibly know each detail before it goes in the copy.
...
- **Reporting in or on mainland China puts the weight on people who stay**: The exposure that matters is not mostly the visiting reporter's. A mainland interviewee can face charges without anything resembling a classic leak: the Counter-Espionage Law as revised in 2023 widened what counts as espionage, the state-secrets law revised in 2024 brought "work secrets" under the same protection regime, and elastic public-order offenses cover the rest, so speaking to foreign media can itself be the exposure. Assume domestic communications are obtainable — WeChat is not end-to-end encrypted, Signal, WhatsApp, and Telegram are blocked, and the VPN a source would need to reach you is itself legally grey. The same applies, and is more often overlooked, to fixers, translators, drivers, and the friend who made the introduction: they carry comparable risk, are rarely treated as "sources," and are still living there after you leave. The risk peak arrives after publication and after your departure, so agree on a no-contact period beforehand, don't thank anyone publicly, and route any decision about someone leaving the country to an organization with local experience. Since July 2024, state-security officers have explicit authority to inspect personal electronic devices, with reported customs spot-checks of phones and laptops in Shenzhen and Shanghai. Verified 2026-08: [FOTN 2025 China](https://freedomhouse.org/country/china/freedom-net/2025){target="_blank"}.
