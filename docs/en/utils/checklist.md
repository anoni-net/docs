...
No passkey yet? Create one on [Passkey as your key](passkey.md) first, or create one right here; both routes end the same way, and if unsure follow the steps on the key page. What a passkey is and where its limits are: [What is a passkey?](../tools/what-is-passkey.md).
...
- The encryption here is only as strong as the place that keeps the passkey: someone who can reach this device and knows the unlock code or the password manager's master password sees what you ticked. In that situation read [digital preparation for survivors of domestic abuse](../scenarios/domestic-violence.md) first, and consider leaving no trace of your preparation on this device at all. If the person you guard against is a partner you live with, law enforcement or a state, do not create a key or save the list on this device. This page has no view-only mode; the items themselves are all in the two scenario articles above, so just read them there. How to tell: [Nothing is saved by default](threat-model.md#Nothing-is-saved-by-default) on the threat model checklist.
