...
1. **Open the site**: use an ordinary browser such as Chrome, Edge, Firefox or Safari to open [anoni.net/docs](index.md), and leave Tor Browser aside for now (the reason is in the [install section](#Install-this-site-as-an-offline-app)). A card slides up from the bottom of the home page asking you to choose a reading language. Pick one. The core chapters for that language then download in the background, about 16 MB of text. If the card does not appear, open any second page in the same language and the download starts just the same.
