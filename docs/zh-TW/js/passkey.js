...
(function () {
  ...
  const STRINGS = {
    "zh-TW": {
      ...
      step1Body: "按下去之後瀏覽器會問你要存到哪裡。存到 iCloud 鑰匙圈、Google 密碼管理員、Bitwarden、1Password 這類會同步的地方，其他裝置才能用同一把。只存在這台裝置上的話，資料也只在這一台解得開。iPhone 沒裝密碼管理器的話選「iCloud 鑰匙圈」，就是手機平常替你記密碼的那個功能。Windows 只用這台電腦的話維持預設的 Windows Hello 就好。視窗由系統顯示，最後會要你輸入登入用的 PIN、指紋或臉。建好之後畫面會列出它能做哪些事，密碼管理器裡也會多一筆叫 anoni.net 的項目。",
      create: "建立 passkey",
    ...
    zh: {
      ...
      step1Body: "按下去之后浏览器会问你要存到哪里。存到 iCloud 钥匙串、Google 密码管理器、Bitwarden、1Password 这类会同步的地方，其他设备才能用同一把。只存在这台设备上的话，数据也只在这一台解得开。iPhone 没装密码管理器的话选「iCloud 钥匙串」，就是手机平常替你记密码的那个功能。Windows 只用这台电脑的话维持默认的 Windows Hello 就好。窗口由系统显示，最后会要你输入登录用的 PIN、指纹或脸。创建好之后画面会列出它能做哪些事，密码管理器里也会多一笔叫 anoni.net 的项目。",
      create: "创建 passkey",
    ...
    en: {
      ...
      step1Body: "The browser will ask where to store it. Choose something that syncs (iCloud Keychain, Google Password Manager, Bitwarden, 1Password) so your other devices can use the same passkey. If it stays on this device only, the data can only be opened here. On an iPhone with no password manager app, pick \"iCloud Keychain\", the built-in feature that already remembers your passwords. On Windows with a single computer, keep the default Windows Hello. The dialog comes from the system and ends by asking for the PIN, fingerprint or face you sign in with. Once created, the abilities the passkey has are listed on this page, and your password manager gains an entry named anoni.net.",
