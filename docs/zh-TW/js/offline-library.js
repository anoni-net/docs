...
(function () {
  ...
  const STRINGS = {
    "zh-TW": {
      ...
      clearing: "清除中",
      cleared: "已清除，自動存下內容也一併關掉了，接下來你讀過的頁面不會留在裝置上。下次連上網時會補回這一頁本身與它需要的樣式（約 0.7 MB），讓你在沒有網路時仍進得來。瀏覽記錄、DNS 快取與你下載過的檔案不在清除範圍內，那些要在瀏覽器或系統裡處理。",
      apply: "套用變更",
      applying: "處理中",
    ...
    zh: {
      ...
      clearing: "清除中",
      cleared: "已清除，自动存下内容也一并关掉了，接下来你读过的页面不会留在设备上。下次连上网时会补回这一页本身与它需要的样式（约 0.7 MB），让你在没有网络时仍进得来。浏览记录、DNS 缓存与你下载过的文件不在清除范围内，那些要在浏览器或系统里处理。",
      apply: "应用变更",
      applying: "处理中",
