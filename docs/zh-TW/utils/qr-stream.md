...
工具會擋下超過 512 KB 的檔案。那個數字量的是人的耐性，不是技術極限。512 KB 用預設設定要切成 1302 張，播完一輪四分二十秒，而收齊通常不只一輪。瀏覽器本身撐得住更多，撐不住的是人。

同樣的 512 KB 換到「極限」檔位只要 180 張，中速播一輪 36 秒。條件夠好的話，這個上限不太會擋到你。

這個通道的天花板在每秒 14 KB 上下。那是「極限」檔位（每張 2944 個位元組）配中速（每秒 5 張）算出來的，而且假設每一張都讀到。理論上把速度也拉到最快是每秒 28 KB，但那一檔的每一張都要花七十毫秒解碼，收的一端跟不上，實際上收不快。

一份 5 MB 的 PDF 照每秒 14 KB 要六分鐘，還是在完全不漏、不重播的前提下，實際上得舉著兩台裝置站十分鐘以上。這種大小的檔案超出了這個工具的範圍，也不該硬做。
...
| 極限 | 2944 個位元組 | 177 × 177 | L | 每秒 14.4 KB | 每秒 28.8 KB |

用預設的「中」就好，這張表是給想知道為什麼的人看的。容錯度那一欄是 QR code 規格裡的等級代號，不需要選，工具會照你挑的檔位自己決定。

「極限」是 QR code 規格能裝的最大值，一張 2944 個位元組。它要對面的相機拍得到 1080p，而且鏡頭要靠近、正面對著、光線要夠。條件不夠的時候它不會給你錯誤訊息，只會一張都收不到，那時候往下退一檔就好。
...
拿相機拍螢幕會出現一圈一圈的水波紋，叫摩爾紋。[QR code 讀取器](qr-read.md)那一頁寫著寧可截圖也不要拍螢幕，就是因為它。這一頁沒得選，只能拍螢幕，所以方格大小跟距離比那一頁更要緊。
...
工具會替檔案算一個校驗碼（SHA-256）放進第一張。校驗碼可以想成檔案的指紋，送的一端先按一次指紋跟著送過去，收的一端拼完再按一次，兩個一樣就代表中間一個位元都沒有出錯。收齊之後畫面上會直接顯示對不對得上。

要注意的是，指紋一樣只證明**檔案沒有在傳輸中壞掉**，不證明**檔案是誰給的**。對方是誰，還是要用別的方法確認，例如當面核對 PGP 指紋，或是回到[已經建立信任的管道](../tools/messaging-comparison.md)再確認一次。

## 印成紙本

按下「印成紙本」會把所有的碼排成 A4 版面，一頁四張，每張下面標著編號。印出來的一疊紙就是那個檔案。

用得上的情況跟螢幕串流不一樣：

- **對面現在不在**。紙可以寄、可以託人帶、可以放進抽屜，收的人幾天後再掃
- **連電力都斷了**。紙不需要電，那是[網路中斷時](../scenarios/shutdown.md)第四級還在運作的少數載體之一
- **手上只有一台裝置**。舉著手機對筆電那套要兩台，印出來就不必

讀回的方式跟螢幕一樣，用另一台裝置打開這一頁的「接收」，鏡頭對著紙上每一張讀過去。順序不重要，每一張都自己帶著編號、總張數與檢查碼，收齊就會拼回原檔。

### 印出來的東西一樣沒有加密

紙上那些方格跟螢幕上的是同一件東西，拍得到的人都讀得回來。而紙比螢幕更容易被拍：它會被放在桌上、夾在資料夾裡、留在影印機的紙匣。敏感的內容先用[本機檔案加密](age.md)封起來再印。

### 張數的上限是 120 張

也就是 30 頁。螢幕播不完只是等久一點，紙本印不完是一疊沒有人會去掃的紙。

超過的時候畫面會顯示這個檔案要印幾頁。把「每張資料量」調到大或極限可以少印很多，代價是方格變密，掃的時候要靠近一點。真的大的檔案不適合走紙本，適合的是金鑰、設定檔、一份聯絡清單這類小東西。

### 印出來的是向量圖

紙上的碼用 SVG 產生，沒有拿螢幕上的畫布放大。雷射印表機是 600 dpi，畫布在螢幕上只有幾百像素寬，放大到 78 mm 邊緣會糊，而糊掉的方格邊界正是解碼器最先失敗的地方。

一張印成 78 mm 見方，極限密度是 177 個方格，一格 0.44 mm，600 dpi 的印表機一格有十個點，手機拍單張讀得回來。
...
這一頁本身要先載入過。第一次打開這一頁需要網路，因為程式要從站上下載。真正要在斷網的場合用它，請事先在有網路的時候打開這一頁並且[存成離線內容](../offline.md)，之後沒有網路也可以開啟。
...
瀏覽器能使用的藍牙功能只設計來跟手環、體重計那類小裝置交換很短的數值，規格裡根本沒有傳檔案這件事。而且網頁只能扮演「主動去連別人」的那一方，沒辦法讓別人來連自己，所以兩個網頁永遠看不見對方。再加上 Firefox 完全沒有實作這個功能，以它為基礎的 Tor Browser 也就沒有。
...
#### 上限是規格的版本 40，而卡住它的曾經是我們自己

決定成敗的是「一個方格在對方相機拍到的畫面裡佔幾個像素」，解碼器要三個像素才勉強分得出一個方格。這個工具原本把相機的畫面壓到 1280 寬再解碼，那樣算下來版本 40 只剩 4 個像素，量出來解不開，所以上限一度訂在版本 25。

那個 1280 是這一頁自己設的，不是相機的限制。改成用相機原生的解析度之後，在真的瀏覽器裡拿 1080p 的畫面實測：

| 版本 | 方格 | 一張裝得下 | 壓到 1280 解碼 | 用 1920 解碼 |
|---|---|---|---|---|
| 25 | 117 × 117 | 1264 個位元組 | 五次全中 | 五次全中 |
| 30 | 137 × 137 | 1723 個位元組 | 五次全中 | 五次全中 |
| 35 | 157 × 157 | 2294 個位元組 | 零次 | 五次全中 |
| 40 | 177 × 177 | 2944 個位元組 | 零次 | 五次全中 |

所以現在開到規格的上限。代價是 1920 的畫面解一次要七十毫秒上下，1280 只要四十，掃描率差將近一倍。工具的做法是先用 1280，連續解不到才升上去，低密度的串流永遠停在快的那一階，只有真的需要的時候才付這個成本。

也就是說版本 40 需要對面的相機拍得到 1080p。做不到的裝置在「極限」那一檔會一張都收不到，往下退一檔就好。
