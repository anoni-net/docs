...
- **停車繳費機上貼著一張 QR code**，看起來跟原本的繳費碼沒兩樣。掃之前先在這一頁確認裡面是不是真的付款頁面，主機名跟官方網域對不上就知道那張貼紙有問題。
...
上面那一格原始內容不遮，一字不差，目的是讓你能確認解出來的到底是什麼。所以要把這一頁截圖給別人之前，先看清楚那一格裡有沒有密碼。`otpauth:` 的密鑰是唯一的例外，欄位表完全不列它。
...
座標的精度那一欄是換算出來的：小數點後每多一位，指到的範圍就縮小十倍。小數第四位在緯度方向大約是 11 公尺，指得到一棟建築。這跟[你的瀏覽器透露了什麼](leaks.md)那一頁說明的位置洩漏是同一件事的兩面。

## 用 Tor Browser 掃可疑的碼

[Tor Browser 的安全等級](../tools/tor-browser-advanced.md)調到 Safest 會關掉 JavaScript，這一頁就整頁不動，而收到來路不明的 QR code 正是那篇建議把等級調高的場合。

先把圖片存下來，切回 Standard 再讀，讀完調回去。這一頁不連外，圖片是在你的裝置上解的。

## 讀不出來的時候

讀不到的時候這一頁不會只試一次。它會換幾種方式再試：先用縮小的版本，接著裁掉外圍把中間放大，再用原始尺寸，最後拉開黑白對比。多數照片在第一種就解完了，只有解不出來的才會往下走。

還是讀不到的話，這幾件事按效果排序：

- **裁掉 QR code 以外的部分**，這個最有效。手機拍的照片動輒四千像素寬，牆上那張碼在畫面裡可能只佔一小塊，裁過之後它就佔滿整張圖
- **靠近一點重拍**，讓碼佔畫面一半以上
- **避開反光**，光斑蓋住定位圖樣就沒得救
