...
小工具區另外送六種事件。它們的用途都一樣：知道哪裡壞了、哪裡該投資源。送出去的一律是固定的代號，沒有檔名、網址，也沒有你輸入或解出來的任何內容。
...
三種方式。把網路關掉再開一次，斷網時一個請求都不會有。用 [Tor Browser](../tools/what-is-tor.md) 的 Safest 等級，JavaScript 全關，這一頁與分析都不會執行。或者用 onion 版，那一版不載入任何分析。
