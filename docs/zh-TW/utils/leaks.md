...
螢幕尺寸的「級距」是 2026-08-23 才變成真的。在那之前頁面上就已經這樣寫，實際送出去的卻是精確的寬高。現在送出前會無條件捨去到百位，1440x900 送出去是 1400x900。這一頁教你螢幕解析度是指紋的一環，我們自己卻收精確值，說不過去。

載入速度也是同一天開始收的，內容是幾個標準指標：首次內容繪製（First Contentful Paint）、最大內容繪製（Largest Contentful Paint）、版面位移（Cumulative Layout Shift）。收它的理由很具體：正體中文版的搜尋索引有 3.6 MB，你第一次按下搜尋就要吞下那一包。在受限或高延遲的網路上那是實際的痛，但要不要動它，目前沒有任何數字可以參考。

每一筆記錄還帶著一個建置代號，例如 `21eb17c`，就是產生這個頁面的那次 commit。改版前後的數字才分得開，看到「載入變慢了」的時候才知道是哪一次改動造成的。那個代號跟你無關，同一次建置的所有讀者共用同一個。
...
`stripmeta-ok` 是 2026-08-23 補的，作用是當分母。在那之前只送失敗，所以看到「這個月 30 筆 unsupported」也不知道算多還是算少。有了成功的計數才算得出失敗率。
...
2026-08-23 起另外送五種，回答的是文件本身該怎麼寫。

| 事件 | 什麼時候送 | 值 |
|---|---|---|
| `search-used` | 有人用了站內搜尋 | 字數級距 `short`、`medium`、`long` |
| `search-zero` | 那次搜尋一筆結果都沒有 | 同上 |
| `search-hit` | 點了搜尋結果 | `first`、`top3`、`rest` |
| `lang-switch` | 用語言選單換了語系 | 來源與目標的語系代號 |
| `read-depth` | 捲到文章的四分之一、一半、四分之三、底部 | `25`、`50`、`75`、`100` |

搜尋關鍵字一個字都不送。你可能正在搜「防火長城」或「翻牆」，那種記錄留在任何地方都是風險，包含我們自己的資料庫。字數級距足以回答「索引好不好用」，關鍵字內容回答不了更多。零結果的比例指向缺哪一類內容，點擊位置指向排序準不準。

語系切換次數是唯一能知道翻譯有沒有人用的訊號，因為文件站不看瀏覽器的語言設定，語系完全跟著你自己選的走。正體中文讀者改看英文版，通常代表譯文或術語有問題，那是要回頭修內容的線索。

閱讀深度回答「這篇要不要拆」。比視窗還短的頁面不量，那種頁面一載入就是底部，量了只會把平均值灌爆。

#### 不想被算進去

clearnet 版可以直接關掉。下面的開關把 `umami.disabled` 寫進這台裝置的 localStorage，Umami 的腳本每次送出前都會檢查那個鍵，有值就一筆都不送。設定只留在這台裝置，清掉瀏覽器資料就會回到預設。

<div id="anoni-optout"></div>

看不到開關的話，多半是你正在看 onion 版，那一版連分析腳本都不會載入，沒有東西需要關。
...
瀏覽器開了 Do Not Track 或 Global Privacy Control 的話，從 2026-08-23 起我們也一筆都不送，下面「Do Not Track 與 GPC」那一節有細節。

所有工具都不直接呼叫分析，一律經過同一個送出口，而那個送出口跟分析腳本寫在同一個區塊裡，onion 與 IPFS 版建置時整段被移除。那兩版的工具因此連送的能力都沒有。

### 我們修掉的一個搜尋詞外洩

2026-08-23 之前，你從站內搜尋結果點進任何一頁，打的字會跟著送進我們的分析資料庫。

原因是兩個各自合理的預設值疊在一起。文件站的搜尋開了關鍵字高亮，Material 因此把命中的詞寫進每一條搜尋結果連結的 `?h=` 參數。Umami 預設把完整網址送出去，包含問號後面那一串。疊起來的結果是搜尋詞離開了你的瀏覽器。

現在每一筆送出前都會先過一道白名單。網址只留得下四個活動來源參數（`utm_source`、`utm_medium`、`utm_campaign`、`utm_content`），其餘的問號參數與 `#` 後面的片段一律清掉，事件名稱或值對不上白名單的整筆丟棄。白名單寫在 [main.html](https://github.com/anoni-net/docs/blob/main/docs/overrides/main.html){target="_blank"}，[測試](https://github.com/anoni-net/docs/blob/main/tools/test_before_send.mjs){target="_blank"}逐條驗它，其中第一條驗的就是這個外洩本身。

2026-08-23 之前的舊記錄裡還留著那些參數，怎麼處理另外決定。
...
這個站從 2026-08-23 起遵守兩者。開著任一個訊號的話，我們的分析一筆都不送。要留意的是別的網站多半不會這樣做，開了訊號在別處換到的通常只有那個區分特徵。
