...
- **國際接軌**：對接 EFF 的 [Tor University Challenge](https://toruniversity.eff.org/zh-tw/){target="_blank"} 計畫，把台灣校園納入全球架設網路
...
- **觀測即時化**：透過 [Pulse](https://anoni.net/api/readme){target="_blank"} 持續追蹤校園節點的運作狀況
...
不需要架過 Tor Relay 也歡迎參與。「我想學」就是合理的加入動機，社群會搭配 [如何搭建 Tor Relay](./setup-tor-relay.md) 文件協助你實際操作。
