---
title: 自我技能評估表
description: 評估自己對 Tor、Tails、OONI 的理解程度，搭配學習路徑指引。給剛接觸匿名網路的台灣志工，作為加入社群協作前的參考量表。
icon: octicons/paste-24
---

# :octicons-paste-24: 自我技能評估表

這裡提供一份自我評估的量表，方便快速定位你對 Tor、Tails、OONI 的熟悉程度。三個工具的角色不同：Tor 是匿名上網的基礎、Tails 把使用情境拉到更高的安全需求、OONI 則是觀測與測量網路審查。你不必依序把三個都點滿，挑跟你想參與的主題相關的來評估就好。如果不知道從哪裡開始學習，每個分級下的「從這裡開始」都列了對應的入門文章，照著補完就能往上一級走。
...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/skill-level-matrix-v4.zh-TW.svg"
        alt="三欄三列的對照表。三欄是 Tor、Tails、OONI，三列是認識、實作、貢獻三個層級。認識層適合任何想了解的人，在 Tor 是說得出洋蔥路由與三層中繼，在 Tails 是說得出跟一般作業系統差在哪，在 OONI 是說得出它在測什麼。實作層適合記者與公民社會工作者，在 Tor 是裝好瀏覽器並會用橋接，在 Tails 是做出開機隨身碟並從 USB 進系統，在 OONI 是執行 Probe 產出檢測報告。貢獻層適合開源社群成員，在 Tor 是架 Relay、Bridge 或 WebTunnel，在 Tails 是備份 Persistent Storage 與用 GnuPG 管理金鑰，在 OONI 是用命令列執行指定測試、整理觀測名單與分析原始資料。三欄各自獨立，不必依序點滿。">
    <figcaption>同一個層級在三個工具上分別代表什麼。三欄各自獨立，不必依序點滿。挑跟你想參與的主題相關的那一欄，從目前所在的那一格往上走。每一格在頁面上都有對應的自我評估清單，還沒到的話，那一級的「從這裡開始」列了要補的文章。</figcaption>
</figure>
...
=== ":material-checkbox-marked-circle-outline: 認識"
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    ??? tip "還沒到這個程度？從這裡開始"
        1. 閱讀「[什麼是 Tor？](../tools/what-is-tor.md){target="_blank"}」（約 5–10 分鐘）
        2. 閱讀「[什麼是匿名網路？](../tools/what-is-anonymity-network.md){target="_blank"}」
        3. 閱讀「[網路自由為什麼重要？](../basics/internet-freedom.md){target="_blank"}」
        4. 完成後回來勾選清單，確認自己的理解。
    ...
    !!! abstract "參考說明"
        ...
        ??? question "Tor 的運作原理。"
            ...
            可以先從「[什麼是 Tor？](../tools/what-is-tor.md){target="_blank"}」章節開始了解。
        ...
        ??? question "網路自由為何重要？匿名網路是什麼？"
            ...
            可以先從「[網路自由為什麼重要？](../basics/internet-freedom.md){target="_blank"}」章節開始了解。
        ...
        ??? question "你所在地區的網路自由現況如何？"
            ...
            1. **國際排名與報告**：「自由之家」（Freedom House）每年發布《網路自由》年度報告，評估各國在網路訪問開放性、言論自由及使用者權利保障方面的表現，是一個很好的參考起點。
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    !!! abstract "參考說明"
        ...
        ??? question "是否可以透過 VPN 連線 Tor？"
            ...
            2. **VPN-over-Tor**：先連接到 Tor，再透過 Tor 使用 VPN。這種設定較少見，需要 VPN 提供商支援透過 Tor 連接，且不一定能對 IP 提供額外保護。
...
=== ":material-checkbox-marked-circle-auto-outline: 貢獻"
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    ??? tip "還沒到這個程度？從這裡開始"
        ...
        3. 參考「[Tor Relays 觀測點](../taiwan/tor-relay-watcher.md){target="_blank"}」，了解如何觀察中繼站的運作狀況。
    ...
    !!! abstract "參考說明"
        ...
        ??? question "使用 Snowflake 瀏覽器擴充套件建立橋接。"
            ...
            3. 可參考專案頁面「[Tor Snowflake](../tools/tor-snowflake.md){target="_blank"}」的詳細說明。
        ...
        ??? question "在 metrics.torproject.org 查詢中繼點狀態。"
            ...
            3. 也可以在「[Tor Relays 觀測點](../taiwan/tor-relay-watcher.md){target="_blank"}」頁面查看本專案整理的視覺化觀測資料。
...
=== ":material-checkbox-marked-circle-outline: 認識"
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    ??? tip "還沒到這個程度？從這裡開始"
        1. 閱讀「[什麼是 Tails？](../tools/what-is-tails.md){target="_blank"}」（約 5–10 分鐘）
    ...
    !!! abstract "參考說明"
        ...
        ??? question "Tails 是什麼，以及它的運作原理。"
            ...
            可以先從「[什麼是 Tails？](../tools/what-is-tails.md){target="_blank"}」章節開始了解。

            Tails 是一個以安全為核心設計的可攜式作業系統，從 USB 隨身碟開機後運行於記憶體中，不在你使用的電腦上留下任何痕跡。所有對外的網路連線預設透過 Tor 網路傳輸。

        ??? question "網路自由為何重要？匿名網路是什麼？"

            這部分的背景知識與 Tor 認識層相同，請參考「[Tor 認識](#Tor-技能分級)」中的對應說明，以及「[網路自由為什麼重要？](../basics/internet-freedom.md){target="_blank"}」章節。
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    !!! abstract "參考說明"
        ...
        ??? question "哪些 Mac 機型無法使用 Tails？"
            ...
            - **完全不支援**：搭載 Apple Silicon（M 系列晶片）的 Mac 無法執行 Tails，因為 Tails 只支援 x86-64 架構。
            - **可能受限**：較舊的 Intel Mac 若有 Apple T2 安全晶片，可能要先在「開機安全性工具程式」放寬安全啟動設定才能從 USB 開機，部分機型仍有已知問題。實際支援情形以 [Tails 官方系統需求](https://tails.net/doc/about/requirements/){target="_blank"} 為準。
...
=== ":material-checkbox-marked-circle-outline: 認識"
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    ??? tip "還沒到這個程度？從這裡開始"
        1. 閱讀「[什麼是 OONI？](../tools/what-is-ooni.md){target="_blank"}」（約 5–10 分鐘）
        2. 閱讀「[網路自由為什麼重要？](../basics/internet-freedom.md){target="_blank"}」
        3. 完成後回來勾選清單，確認自己的理解。
    ...
    !!! abstract "參考說明"
        ...
        ??? question "OONI 是什麼。"
            ...
            可以先從「[什麼是 OONI？](../tools/what-is-ooni.md){target="_blank"}」章節開始了解。
        ...
        ??? question "OONI 的檢測運作方式。"
            ...
            - OONI Probe 會定期發送請求至多個網站和服務，確認[名單上](../taiwan/ooni-checklist.md){target="_blank"}的網站是否可以正常訪問。
        ...
        ??? question "不同地區在網路監視與審查上的差異。"
            ...
            - 可參考「[網路自由為什麼重要？](../basics/internet-freedom.md){target="_blank"}」章節。
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    !!! abstract "參考說明"
        ...
        ??? question "ASN 自治網路的運作。"
            ...
            - 可參考「[ASNs 自治網路觀測資料分析](../taiwan/ooni-asn-coverage.md){target="_blank"}」中的介紹。
...
=== ":material-checkbox-marked-circle-auto-outline: 貢獻"
    ...
    **自我評估**（勾選你已能做到的項目）：
    ...
    ??? tip "還沒到這個程度？從這裡開始"
        1. 先完成「實作」層的所有項目。
        2. 閱讀「[OONI 網站檢測清單](../taiwan/ooni-checklist.md){target="_blank"}」，了解名單的收錄方式與現況。
        3. 閱讀「[ASNs 自治網路觀測資料分析](../taiwan/ooni-asn-coverage.md){target="_blank"}」，了解原始資料的分析方式。
    ...
    !!! abstract "參考說明"
        ...
        ??? question "網站觀測名單的收錄方式。"
            ...
            可參考「[OONI 網站檢測清單](../taiwan/ooni-checklist.md){target="_blank"}」了解更多。
        ...
        ??? question "如何協助整理與更新名單。"
            ...
            4. 詳細流程可參考「[OONI 網站檢測清單](../taiwan/ooni-checklist.md){target="_blank"}」中的說明。
...
- **認識層**：先訂閱 [電子報](../contact.md) 跟讀社群動態，或到 [指南](../guides/index.md) 把概念補齊。
- **實作層**：照各工具分級下的「從這裡開始」實際操作安裝、日常操作，遇到問題到 Matrix 發問。
- **貢獻層**：到 [如何參與與認領主題](./how-to-contribute.md) 選一個方向，在 Matrix 對應 room 說明自己的程度與想做的事，社群會協助你接上手。帳號申請與服務入口見 [社群自架服務](./tools.md)。
