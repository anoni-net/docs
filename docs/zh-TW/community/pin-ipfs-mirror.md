...
anoni.net 文件站除了主站，還有一份 IPFS 鏡像，讓文件在主站被封鎖或下架時仍然讀得到。IPFS 上的內容要有節點 pin 才會存活。2026 年 8 月查過網路上的提供者，只有社群自己那一台，整份鏡像目前是單點。你多架設一個節點幫忙 pin，網路上就多一份完整複本，那台機器被停權或出事的時候文件還在。
...
!!! tip "要幫忙，你需要"
    ...
    第一次要抓的整份鏡像約 240 MB，來源目前只有社群那一台，建議一開始就設好 [跟社群節點保持常連](#peering)，抓起來會順很多。
...
=== "Windows"
    ...
    最簡便的是安裝 [IPFS Desktop](https://docs.ipfs.tech/install/ipfs-desktop/){target="_blank"}，它內含 kubo，登入後會自動在系統匣常駐，daemon 一直開著。IPFS Desktop 從 2026 年 9 月 30 日起沒有專職維護者，見 [2026 年 9 月之後的 IPFS 維護變化](#ipfs-maintenance-2026)，想少一層不確定的話，Windows 也可以裝獨立版 kubo 或用 Docker。
...
peering 是單向設定，社群節點那端沒有對應條目，連線的保活責任落在你這邊。[kubo 官方文件](https://github.com/ipfs/kubo/blob/master/docs/config.md#peering){target="_blank"} 提醒過，單向 peering 會佔用對方節點的連線資源，鏡像數量變多之後負擔會集中在同一台。這個顧慮成立的前提是幫忙 pin 的節點夠多，而現在提供者只有一台，第一次那 240 MB 全靠它，所以建議第一次設定就打開。等 pin 的節點多起來、來源不再集中在同一台，再拿掉也不遲。

## 2026 年 9 月之後的 IPFS 維護變化 { #ipfs-maintenance-2026 }
...
- **每次更新只抓差異**：CID 每次發布都會變，但腳本是先 pin 新版成功、才放掉舊版。pin 新版的當下舊版的區塊還在你的節點上，IPFS 只去抓真正不一樣的區塊。第一次要抓整份，之後一般的內容更新通常只有幾 MB。動到導覽列或版型的那幾次例外，全站 HTML 會一起重寫，差異接近一半的鏡像大小。
- **磁碟不會一直長大**：腳本 unpin 舊版後會執行一次垃圾回收，只留最新版本佔空間。一份完整鏡像約 240 MB（2026 年 8 月實測）。
