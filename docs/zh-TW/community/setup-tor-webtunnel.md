...
WebTunnel 是 Tor 目前抗審查能力最強的橋接方式之一。它把 Tor 流量包進一個正常的 HTTPS 連線裡，流量形態與一般 HTTPS 瀏覽近似，難以被深度封包檢測（DPI）單獨挑出來。當審查系統用 DPI 封鎖 obfs4 這類「看起來不像 HTTPS」的協定時，WebTunnel 通常仍能穿過去。
...
- **回應真實需求**：2026 年伊朗在軍事行動期間多次大規模斷網，第二階段持續近三個月（2 月底至 5 月底）。重新開放後，社群自架的 WebTunnel 觀察到流量明顯湧入，記錄在 [伊朗斷網後：流量湧入社群的 WebTunnel](../blog/posts/iran-blackout-webtunnel.md)。在極端審查下，這種偽裝成 HTTPS 的橋接，往往是當地人能否連上 Tor 的關鍵。
