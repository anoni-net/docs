...
??? question "Tor 不是會讓犯罪份子做壞事嗎？"
    ...
    **犯罪的意圖在於人，不在工具。** 加密、電話、現金都會被惡意使用，Tor 也一樣。如果移除 Tor，有惡意的人會改用其他工具，並不會因此停止。
...
??? question "那分散式阻斷服務攻擊（DDoS）呢？"
    ...
    Tor 僅傳輸正確的 TCP 連線，不傳輸所有 IP 封包。**Tor 無法傳送 UDP 封包，也無法進行 SYN Flood 等特殊攻擊**。常見的 DDoS 攻擊無法透過 Tor 完成。
...
??? question "那垃圾郵件（Spam）呢？"
    ...
    許多垃圾郵件技術（例如偽造的 UDP 封包）無法透過 Tor，因為 Tor 僅支援 TCP。
