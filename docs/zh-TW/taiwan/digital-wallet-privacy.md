...
監察院同年三月的調查報告寫著，內政部規劃過程規避既有法制、授權不足[^28]。
...
數發部的立場是兩者不同，官方文件反覆寫著數位憑證皮夾並非發行數位身分證，只是整合多種數位證件的容器。技術上這個區分成立，皮夾本身確實不發行身分識別。爭點在於 2021 年那三個條件是對「數位身分」這件事說的，而不是對某一張證件說的。
...
| 問題 | 答案 | 依據 |
|---|---|---|
| 發證方看得到你何時何地出示嗎 | 看不到 | 出示時不回連發證方，撤銷狀態查的是一份十三萬張憑證共用的清單 |
| 不同的驗證方能串連你嗎 | 能 | 沒有批次發行一次性憑證，SD-JWT 的簽章固定，零知識證明官方列為未來工作 |
| 誰能要求你出示 | 會查但不會擋 | `client_id` 用 `DID` 形式，皮夾查得出對方的公鑰，信任清單未列入時只顯示警告（錯誤碼 `4012`）並繼續流程 |

三題的答案不同，籠統問「皮夾安不安全」得到的會是三題混在一起的印象。
