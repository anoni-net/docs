  ...
  function fetchSummaryTable() {
    ...
    Promise.all(COUNTRIES.map(function (c) {
      ...
      if (caption && results[0].data.length) {
        ...
        caption.textContent = '資料日期：' + latestDay;
      }
    }).catch(function () {
      tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--md-default-fg-color--light);">載入失敗</td></tr>';
    });
  }
...
此頁面持續追蹤各地區 Tor 中繼節點的運作狀況，資料每小時從 Tor 網路收集一次，以天為單位彙整，涵蓋健康度、多樣性與版本分布等面向。
...
自治網路（ASN）的分散程度，以及不同節點角色的分布狀況，反映網路的去中心化程度，越分散越不容易被單一節點或單一電信商的問題一次影響。
...
Tor 軟體版本的更新狀況，以及各節點所具備的能力標籤（Flags，Tor 網路給節點貼的角色標記，例如作為電路起點的 Guard、負責連往目標站的出口 Exit、長期穩定的 Stable）分布。
...
<table>
  <thead>
    <tr>
      <th>地區</th>
      <th style="text-align:right;">運作中</th>
      <th style="text-align:right;">已停止</th>
    </tr>
  </thead>
  <tbody id="summary-table-body">
    <tr><td colspan="3" style="text-align:center;padding:.6rem;color:var(--md-default-fg-color--light);">載入中...</td></tr>
  </tbody>
</table>
...
[^1]: 持續與停止運作的數量：計算每日該地區的 Tor Relay 運作狀態數量，計算區間每小時不重複的 Tor Relay。
[^2]: 每日提供的總頻寬：計算每日該地區**運作中**的 Tor Relay 總頻寬（MB/s），計算區間每小時不重複的 Tor Relay。
[^3]: 版本數量：計算每日該地區 Tor Relay 所使用的 Tor 版本號數量，計算區間每小時不重複的 Tor Relay。
[^4]: ASN 每日唯一數量：計算每日該地區的 Tor Relay 自治網路編號（ASN）的唯一數量，數值越高，代表節點分散於更多不同的網路供應商，分布越均勻。
[^5]: 中繼點類型數量：計算每日該地區的 Tor Relay 類型數量，計算區間每小時不重複的 Tor Relay。
[^6]: 標籤類型數量：計算每日該地區的 Tor Relay 標籤類型數量。各標籤不互斥，同一節點可同時具備多個標籤。
