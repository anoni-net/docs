...
要知道台灣的網站有沒有被審查、連線正不正常，得先有一份「要測哪些網站」的清單。[OONI](../tools/what-is-ooni.md) Probe（公民自己就能跑的網路審查檢測工具）每次檢測，都照著一份事前列好的網站清單，逐一檢查每個網址連得上連不上。清單收錄了什麼、由誰維護、多久更新一次，直接決定 [ASN 覆蓋率分析](./ooni-asn-coverage.md) 這類後續觀測能不能反映真實情況。台灣這份清單大多停在 2017 年，這頁說明它的現況、如何維護，以及不用寫程式也能參與的地方。
