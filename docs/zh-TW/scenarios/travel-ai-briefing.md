...
!!! warning "AI 會編造電話、法條與資費"
    語言模型很會「說得很有把握是錯的」。把 AI 的回答當成「我接下來該查哪些東西」的起點，不要當成最終答案。尤其是緊急電話、法條編號、簽證規則與資費，**一定**回到官方一手來源核對。要注意一個反差：目的地越冷門、公開資料越少，AI 手上的訓練資料也越少、越容易亂編，偏偏這種地方最需要可靠資訊。這種時候更要把 AI 的回答只當成「該查哪些東西」的清單，逐項拿去比對本頁底部的一手來源。

<figure markdown="span">
    <img src="https://assets.anoni.net/diagrams/travel-briefing-flow.zh-TW.svg"
        alt="四個並排的步驟，用箭頭串起來。第一步是開場 prompt，把出發地、目的地與停留天數換成你的資訊，貼給 AI 設定角色，只需要填一次。第二步是五項調查，逐項複製貼上即可，內容是數位環境（審查、Tor、VPN、設備搜查）、法律與政治風險、連線與通訊（SIM 與 eSIM）、緊急聯絡與支援網路，以及選用的實體與監控環境。第三步是核對一手來源，拿 AI 的答案去對照文章底部的官方來源，緊急電話、法條編號、簽證規則與資費一定要回到一手來源核對。第四步是隨身緊急卡，把第四項問到的關鍵聯絡抄成卡片，印出來過塑，皮夾與行李各放一張，並同步存進手機離線筆記。最下方的紅框提醒 AI 會編造電話、法條與資費。">
    <figcaption>AI 的回答是「該查哪些東西」的起點</figcaption>
</figure>
...
這裡每個區塊是一題。一題一題來，複製一題、送出，看 AI 的回答並對照本頁底部的來源，再貼下一題。開場 prompt 已經設定成逐題回答，分開問每題會答得更完整，也比較好逐項查證。整批一次貼上，AI 容易把問題合在一起、給一個籠統的回答，反而難逐項查證。
...
執行完上面幾項後，把第四節「緊急聯絡與支援網路」問到的關鍵聯絡抄成一張卡片，印出來過塑、皮夾與行李各放一張，並同步存進手機離線筆記與（若你用）Tails。下面是空白模板：
...
- **Tor Metrics**（Tor relay、bridge、各國連線數）：<https://metrics.torproject.org/>{target="_blank"}（anoni.net 自己的 [Tor relay watcher](../taiwan/tor-relay-watcher.md) 也用同一套 Onionoo 資料）
