---
title: 出國前數位安全：用 AI 自助產生目的地 briefing
description: 複製這些 prompt 去問你自己的 AI，出發前先摸清目的地的網路審查、法律、SIM 與緊急聯絡。適用任何國家，你的查詢不會經過我們。
icon: material/shield-airplane-outline
---

# :material-shield-airplane-outline: 出國前數位安全：用 AI 自助產生目的地 briefing

出國前你會查簽證、插頭、換匯，但很少有人查「我要去的地方，網路被怎麼管？我的工作在那裡合不合法？出事找誰？」對記者、人權工作者、NGO 成員、研究者與社運參與者來說，這些才是真正影響安全的問題。

這一頁**不是**一份各國資料表。那種預先整理好的版本請看 [出差與研討會的數位準備（東亞與東南亞）](./asia-travel.md)。這一頁適用**任何目的地**：我們給你一組可複製的提問（prompt），你把它貼進**自己的 AI**，由 AI 幫你產生一份行前的數位與人身安全 briefing。
...
1. **填空**：把下面 prompt 裡的 `{出發地}`、`{抵達地}`、`{停留天數}` 換成你的資訊（日期與身分可留白）。
2. **先設定角色**：先把「開場 prompt」貼給 AI，再逐項貼下面的問題。
...
=== "低資料版（雲端 AI）"
    ...
    我要前往 {抵達地}（國家層級即可），停留時間以「短期停留」為準。
    我不會提供姓名、組織或精確日期。

    接下來我會逐項貼問題。每一項請引用可查證的一手來源並附連結、
    區分事實與研判、不確定就說不確定、結尾給一句行動建議。
    ```
...
```text
【數位環境 1／審查與封鎖】請查 OONI Explorer 關於 {抵達地} 最近 6–12 個月的
...
```text
【數位環境 2／Tor 可達性】{抵達地} 目前有多少運作中的 Tor relay 與 bridge、
...
```text
【數位環境 3／VPN 合法性與可用性】在 {抵達地} 使用 VPN 是否合法？WireGuard、
OpenVPN 等協定是否被封鎖或限速？是否只允許政府核可的 VPN？出發前該先裝好
哪些、並準備哪些備援？
```

```text
【數位環境 4／邊境與設備搜查】入境 {抵達地} 時，海關是否可能搜查手機、筆電，
或要求登入社群帳號？當地是否有強制交出密碼或加密金鑰的法律？持有或使用
加密通訊 app 是否違法？是否有記者或人權工作者被拒入境、被拘留的紀錄？
```

```text
【數位環境 5／網路關閉風險】{抵達地} 過去是否發生過 internet shutdown 或
...
```text
【法律與政治 6／旅遊警示】{出發地} 外交部（領事事務局）目前對 {抵達地} 發布
的旅遊警示燈號是什麼？對照美國國務院、英國 FCDO 的 advisory 現況。近期是否
有政治動盪、抗議或衝突？附官方連結與更新日期。
```

```text
【法律與政治 7／對我角色的法律風險】針對記者／NGO／行動者，{抵達地} 的法律
...
```text
【法律與政治 8／入境與身分】持 {出發地} 護照入境 {抵達地}，簽證或電子旅行
許可（ETA／ETIAS 等）要求為何？停留 {停留天數} 天是否在免簽額度內？
{出發地} 與 {抵達地} 的外交關係是否會影響我能得到的領務協助？
```

### 三、連線與通訊（SIM、eSIM）

```text
【連線與通訊 9／SIM 與 eSIM】在 {抵達地} 購買實體 SIM 是否需要實名登記
（護照／生物辨識）？依我停留 {停留天數} 天、從 {出發地} 出發，建議用區域型
或單國 eSIM（如 Airalo）還是原號漫遊？請列幾個方案、大概價格與接取的當地
電信，並提醒：語音盡量走 Signal／WhatsApp 等 VoIP，少用傳統漫遊通話。
```

### 四、緊急聯絡與支援網絡

```text
【緊急聯絡 10／領務代表處】{出發地} 在 {抵達地} 的代表處／大使館的「急難救助」
24 小時電話、一般聯絡電話、地址與上班時間為何？若 {抵達地} 沒有設館，最近的
兼轄館處是哪一個？請整理成可抄寫的格式。
```

```text
【緊急聯絡 11／數位安全與在地支援】請列出 Access Now Digital Security Helpline
的聯絡方式（24/7、多語、help@accessnow.org，會在兩小時內回覆）。{抵達地} 有
哪些在地的數位人權、記者保護或法律支援組織？裝置被扣或帳號被攻擊時該找誰？
```

```text
【緊急聯絡 12／當地緊急號碼與法律援助】{抵達地} 的報案、救護、消防號碼各是
...
```text
【實體環境 13／監控與治安（選用）】{抵達地} 的公共監控程度如何（CCTV、
