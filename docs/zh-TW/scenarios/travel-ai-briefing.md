...
這裡每個區塊是一題。一題一題來，複製一題、送出，看 AI 的回答並對照本頁底部的來源，再貼下一題。開場 prompt 已經設定成逐題回答，分開問每題會答得更完整，也比較好逐項查證。整批一次貼上的話，AI 容易把所有問題合在一起給一個籠統的回答，反而難逐項查證。
...
!!! tip "想讓 AI 查到真實的 Tor 節點數字，先接 onionoo MCP"
    這題問的 relay、bridge 數量與頻寬，AI 若沒有外部資料來源，很可能憑記憶編一個給你（就是這頁開頭警告的那種錯）。社群自架的 [onionoo MCP](../community/onionoo-mcp.md) 把 Tor Project 的 Onionoo 公開資料接成一個網址，在 claude.ai 這類 AI 助理貼上去，AI 就能查到查詢當下、可引用的真實數字。
...
跑完上面幾項後，把第四節「緊急聯絡與支援網絡」問到的關鍵聯絡抄成一張卡片，印出來過塑、皮夾與行李各放一張，並同步存進手機離線筆記與（若你用）Tails。下面是空白模板：
...
- [出差與研討會的數位準備（東亞與東南亞）](./asia-travel.md)：14 地已預填好審查、VPN、SIM 與入境查機，只去東亞東南亞、或趕時間不想自己跑 prompt 時，直接查那篇更快。
