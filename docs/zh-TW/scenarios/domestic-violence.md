...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/tech-abuse-surface-v2.zh-TW.svg"
        alt="四個並排的方框，列出親密關係中的科技濫用最常走的四類管道，全部都是為家庭與伴侶設計的一般功能。第一類是帳號存取與共用方案，包含對方知道或猜得到的密碼、共用的 Apple ID 或 Google 帳號、Apple 家人共享與電信家庭方案、Google Family Link，以及密碼重設信寄到共用號碼或共用信箱這條救援管道。第二類是位置追蹤，包含尋找與 Google 地圖的位置分享、AirTag 這類藍牙追蹤器、車聯網 App、共用日曆與照片的地理標記。第三類是裝置上的追蹤類 App，在對方短暫取得手機時安裝，暗中轉發訊息、通話紀錄、照片、瀏覽行為與位置，而且可以完全不顯示圖示。第四類是智慧家庭與共用裝置，包含智慧鎖、攝影機、門鈴、溫控、語音助理與共用的串流帳號。下方的藍框指出最可靠的訊號往往是對方知道了他不該知道的事，並提醒離開受暴關係的前後是最危險的時期。">
    <figcaption>走的都是為家庭與伴侶設計的一般功能。先盤點再處理。在還沒確認裝置乾不乾淨之前，不要用那台裝置查求助資源或跟社工聯絡。證據要保留，不要為了「清乾淨」而大量刪除騷擾訊息，也不要存進對方看得到的雲端。</figcaption>
</figure>
...
- **完整擷取上下文**。截圖要含訊息本身、寄件者、時間戳記與前後對話。平台允許的話匯出原始檔。用另一支相機拍下螢幕也比什麼都沒有好。原始截圖留著不動，要把副本交給社工與律師以外的人，或公開求助時，先用[截圖遮蔽](../utils/redact.md)蓋掉第三人的名字與頭像
