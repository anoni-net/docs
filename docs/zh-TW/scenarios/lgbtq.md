...
每個場景的對手不同，但都圍繞同一個核心問題。我哪些數位足跡被串起來，會把不想連結的兩個身分連起來。下面五個小節把這個問題拆成可以動手的設定。

!!! tip "現在最怕家人翻到手機？先做這三件事"
    如果你還在櫃中、住在家裡，最急的處境通常是家人拿到手機就看到。可以先跳到 [家人控管下的本機痕跡](#家人控管下的本機痕跡) 一節，三件事最快見效：關掉鎖定畫面的通知預覽、付費改用禮物卡或點數而不要綁信用卡、把交友 app 圖示藏進資料夾深處。其餘帳號分流、跨平台反查等較完整的整理，等比較有空再回頭看。

## 帳號分流：日常、社群、探索

身分區隔的基礎是「不同層的帳號不要共用 email、不共用 username、不共用瀏覽器、不共用密碼管理器資料夾」。這聽起來繁瑣，一次設定好之後就照流程跑。
...
- 註冊 alt account 時手機門號用網路虛擬號碼（TextNow、付費的 MySudo），避免用本人 SIM 卡。WhatsApp 近年大量封鎖這類 VoIP（網路電話）門號，註冊前先確認。曾被推薦的 JusTalk 在 2022 年被揭發所謂「端對端加密」其實是假的，數百萬則明文訊息、虛擬門號與真實號碼的對應關係外洩、暴露在公開網路上[^5]，不建議再使用
- 頭像用社群代號或抽象圖案，不要用本人臉
