...
<figure markdown="span">
    <img src="https://assets.anoni.net/diagrams/activist-timeline.zh-TW.svg"
    ...
    <figcaption>三個階段的準備門檻都不高，卡關多半是拖到當天</figcaption>
</figure>
...
3. SIM 卡用預付卡。要注意台灣的預付卡同樣要實名登記，能做的是不要登記在跟主門號同一個人名下。跨境情境的取捨見 [出差與研討會的數位準備](./asia-travel.md) 的乾淨機一節
4. 平時關機，行動當天才開機
...
Google Docs、Notion、騰訊文件這類雲端共筆好用，但有兩個問題：平台側可以調閱內容、第三方取得分享連結就能看到。動員前把敏感共筆先搬家：
...
數位足跡審計不只服務這次動員，更是長期的數位衛生。每次行動前抽 30 分鐘檢查，會比事後處理輕鬆很多。

### 活動公開資訊的揭露節奏

公開活動要有人來，資訊本來就需要對外公開。可以調整的是哪一項資訊什麼時候發，以及發給誰。

2026 年 9 月 Anthropic 的濫用偵測報告記錄了一批被封禁的帳號，與中國市級公安及國安機關有關，其中一項作業產出境外活動的前期場地情報，內容包含集合點、路線與終點。蒐集對象是溫哥華的一場民主遊行、土耳其的維吾爾文化活動與奧斯陸自由論壇的放映場次，三者都是合法公開的活動，蒐集發生在活動之前。報告寫明這一項是該批案例裡最嚴重的一條，逐段整理見 [Anthropic 九月威脅報告寫到台灣的四個段落](../blog/posts/2026-anthropic-threat-report-taiwan.md)。

把活動資訊分成三層，各自對應一個時間點：

- **從一開始就公開**：日期、主題、大致區域（台北市中正區這種精度）、報名連結、聯絡窗口
- **前一天才給報名者**：精確地址、樓層與入口、集合的確切時間
- **只有現場工作人員知道**：遊行路線與備援路線、退場動線、醫療與法務的位置、核心成員的離場時間

幾種常見的漏法：

- **報名頁與社群貼文用同一份文案**：報名頁可以要求填資料才看得到地址，社群貼文對所有人公開，兩邊的精度應該不同
- **場地平面圖直接貼出來**：租借場地的平面圖、逃生動線圖與座位配置圖，多數參加者不需要事前看到
- **共用行事曆與地圖連結**：主辦單位的公開行事曆、Google Maps 的分享連結、共筆文件的公開連結，都會把地址帶出去，而且會留在網路上
- **現場直播與即時打卡**：即時揭露的是位置與人數，延後兩小時發同一批內容，多數情況下沒有損失

有固定會址的組織，包含教會、協會與長期辦公開聚會的 NGO，重點落在盤點。自家網站、活動頁與過往的活動紀錄上已經放了什麼，比單場活動的節奏更值得先看一次。

這一節處理的層次跟站上多數工具不同。蒐集者用的是你自己公開出去的資訊，端對端加密、VPN 與 Tor 對它沒有作用，能調整的只有揭露的時機與精度。
...
手機在現場可能被搜或被扣，預設假設「取得手機的人就取得所有東西」。降低這個假設的成立程度：
...
- 多數動員場景行動數據夠用。真正的核心是群組設計、訊息政策、誰知道誰的號碼，工具選擇反而次要
...
- 要公開或交給媒體的截圖，先用[截圖遮蔽](../utils/redact.md)蓋掉其他成員的名字與頭像，原始檔留著
...
2026 年 9 月 Anthropic 的濫用偵測報告第 82 到 85 頁記錄了一家商業情報廠商用模型建監控平台的案例。報告第 83 頁引述 Forbidden Stories 在 2023 年對該公司外流簡介所做的調查，服務內容包含建立假帳號滲透私人的 WhatsApp 與 Telegram 群組、收割成員清單，再升級到釣魚與入侵裝置。Anthropic 在試辦階段就發現並封禁了帳號，沒有證據顯示後續階段對真實目標使用過，也無法獨立確認 Forbidden Stories 報導的下游階段。逐段的頁碼對照見 [Anthropic 九月威脅報告寫到台灣的四個段落](../blog/posts/2026-anthropic-threat-report-taiwan.md)。

跨組織與跨國的協作靠這類私人群組維持，成員驗證值得當成例行工作：

- 拉人進群之前，先確認是誰為這個人背書，引薦人留下紀錄
- 新成員加入時公告是誰邀請的，讓其他成員有機會提出疑問
- 每季或每個專案結束後盤點一次成員清單，離開專案的人一併移出
- 群組裡不放完整的聯絡名單與代號對應表，那正是收割的目標
- 長期沉默的帳號突然活躍並索取資料時，換一個管道確認本人

同一套辨識也適用在冒充你的情況。你的帳號被複製時，能保護身邊的人的是事前約好的驗證方式，作法見 [記者保護消息來源](./journalist.md) 的你自己是目標的時候一節。
...
- **法規量級不同**：香港《國安法》（2020）與 2024 年《維護國家安全條例》（基本法 23 條立法）下，煽動罪最高刑期已從 2 年提高到 7 年（涉境外勢力達 10 年），另有分裂國家、顛覆、境外干預等罪，最重可判終身監禁。參與集會、群組通訊、裝置內容都可能成為國安案件的證據，國安處可扣押裝置作鑑識，跟台灣以罰鍰為主的集遊法、社維法不是同一量級。
- **本地求助管道大幅萎縮**：香港民間人權陣線、職工盟等組織已於 2021 年在國安壓力下解散，「找本地 NGO 就有支援」在香港多半已不成立。數位安全與風險評估建議轉向國際組織，如上面已列的 [Access Now Helpline](https://www.accessnow.org/help/){target="_blank"} 與 [Citizen Lab](https://citizenlab.ca/){target="_blank"}。人身安全若有立即風險，優先考慮離開當地或聯繫國際人權組織的緊急協助。

查證來源（2026-07）：[Article 23: Hong Kong's 2024 security law](https://hongkongfp.com/article23-security-law/){target="_blank"} - Hong Kong Free Press、[Safeguarding National Security Ordinance](https://en.wikipedia.org/wiki/Safeguarding_National_Security_Ordinance){target="_blank"} - Wikipedia。
