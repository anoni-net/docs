...
- 兩階段驗證優先用 [TOTP](https://en.wikipedia.org/wiki/Time-based_one-time_password){target="_blank"}（Authy、Aegis、Bitwarden 內建），不要只用簡訊
...
- 台灣人權促進會[^3]
- 法律扶助基金會[^4]
- 國際：[Access Now Helpline](https://www.accessnow.org/help/){target="_blank"}（24/7 多語）、[Citizen Lab](https://citizenlab.ca/){target="_blank"}（裝置鑑識）
...
[^1]: 2020 年研究團隊指出 Bridgefy 加密設計缺陷，見 [Bridgefy, the messenger promising secure, private chat for activists, was a bug-ridden mess](https://blog.cryptographyengineering.com/2020/08/24/anatomy-of-a-bad-idea-bridgefys-broken-encryption/){target="_blank"} - Cryptography Engineering blog
