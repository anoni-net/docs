...
原始資料來自 Apple 的[安全性更新發布頁](https://support.apple.com/en-us/100100){target="_blank"}。沒有出現在那一頁的小改版，本頁不另立條目。出現在那一頁、Apple 卻註記沒有公布 CVE 項目的版本，會立一則簡短條目，讓讀者知道自己手上還不是最新版。
...
> 2026-09-08 · [安全性更新發布頁](https://support.apple.com/en-us/100100){target="_blank"}

- <span class="urg-tag urg-tag--routine">一般</span>Apple 把這一版列進安全性更新發布頁，那一列的註記說明這次更新沒有公布任何 CVE 項目，也沒有附上安全內容說明的連結。沒有標注已被實際利用的問題。
- 沒有明細就看不出修了什麼，跟著平常的節奏更新即可。Apple 之後補上安全內容說明時，這一則會改寫。
- 同一天沒有 macOS 與 18.x 線的更新。還在 18.x 的機型最新仍是 8 月 17 日的 18.7.10。
...
- 18.x 線補 49 個，其中兩個是值得注意的隱私問題：app 可能繞過 App 隱私報告的記錄（那份報告本來就是拿來稽核 app 在背後連了哪裡），以及 app 可能列舉裝置上已安裝的應用程式（可用來側寫使用者身分）。另有一個 Wi-Fi 元件的問題讓 app 可能以核心權限執行任意程式碼，那一個的後果比隱私外洩更嚴重。
