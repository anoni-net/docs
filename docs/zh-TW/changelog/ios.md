...
- **立刻**：Apple 在公告裡標注該問題可能已被實際利用，或漏洞被美國 CISA 的已知遭利用漏洞目錄收錄。看到這一級，當天就更新。
- **儘快**：修補涵蓋 WebKit 或 Kernel 的記憶體損毀類問題。前者是造訪網頁就可能觸發的環節，後者決定攻擊者能取得多少權限，兩者串起來就是一條完整的遠端攻擊鏈。幾天內更新。
- **一般**：其餘修補，跟著平常的節奏更新即可。
...
- 急迫程度：儘快。沒有標注已被實際利用的問題，不過 WebKit 與 Kernel 都在修補範圍內。
...
- 急迫程度：儘快。86 個修補，Kernel 佔 19 個、WebKit 佔 7 個、ImageIO 佔 6 個。
...
- 急迫程度：儘快。38 個修補裡有 24 個在 WebKit、4 個在 WebRTC，整包幾乎都是瀏覽器引擎。
...
- 急迫程度：儘快。26.x 線補 67 個，WebKit 佔 21 個、Kernel 佔 6 個。Kernel 那組有一個 app 可能取得 root 權限。
