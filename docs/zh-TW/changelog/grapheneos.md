...
> 版本 `2026090500`、`2026090700`、`2026091000`、`2026091700`、`2026091900` · [官方發布頁](https://grapheneos.org/releases){target="_blank"}

- 安全修補等級在 9 月 10 日推進到完整的 2026-09-01 Pixel 等級。Google 的九月公告在 9 月 8 日恢復列出漏洞明細，該月的內容整理在 [Android 安全修補等級](./android.md)。
- 9 月 15 日的 Pixel 更新公告寫明行動數據機的 CVE-2026-58704 可能正遭到針對性的利用（細節見 [Android 安全修補等級](./android.md)），Google 的修補在 2026-09-05 修補等級。GrapheneOS 到 9 月 19 日那版為止的發布說明都沒有寫到 2026-09-05，也沒有提到這個 CVE。9 月 17 日那版從 Android 17 QPR1 backport 了行動數據機韌體，有沒有涵蓋這個修補，發布說明沒有交代。
- 電話（Dialer）加入自動通話錄音。9 月 5 日先做成每通可以退出的開關，來電與去電都涵蓋，通話中會顯示正在錄音的提示。9 月 10 日補上「預設對所有聯絡人錄音，另外維護一份排除號碼清單」的選項，同時宣告麥克風與電話的前景服務類型，修好用藍牙接聽時錄音不會啟動的問題。9 月 17 日起，第一次開啟自動錄音的人預設就套用這個排除清單的模式，原本的預設是逐一加入要錄的聯絡人。通話錄音在許多地區涉及法律問題，開之前先確認自己所在地的規定。
- 私密空間（Private Space）補了兩處上游的隱私缺口。鎖定狀態下應用程式抽屜的搜尋不再列出私密空間裡的 app，Pixel Launcher 是靠 Android System Intelligence 處理搜尋才沒有這個問題。設定裡另外加上警告，說明 Android 內建的隱藏私密空間只是表面遮蔽，已知有多種偵測手法。把私密空間當成藏得住的人要重新評估。
- 9 月 17 日加上剪貼簿的讀取限制，可以逐一 app 設定，也可以設成全域預設。開啟後 app 不能自行讀取剪貼簿，只能透過系統的選取工具列、鍵盤、快捷鍵或無障礙服務貼上，每次都要使用者親自操作。剪貼簿常放著密碼與一次性驗證碼，這個限制讓背景的 app 讀不到那些內容。擋下讀取時會跳通知，9 月 19 日把同一則通知的最短間隔從 1 分鐘縮到 6 秒，嫌吵的人可以逐一 app 關掉提示。內建鍵盤同時加上貼上建議，最近複製的內容會出現在建議列上。
- hardened_malloc（GrapheneOS 自己的強化記憶體配置器）改用 `MADV_DONTNEED_LOCKED`，跟 `mlockall` 的相容性變好，並修掉極端記憶體不足時可能解除對映卻換不上新對映的邊界情況。
- 系統檔案管理員（DocumentsUI）不再對複製與部分移動的檔案抹掉位置中繼資料。那是上游的錯誤，原本會讓人以為自己保留了完整的原始檔。
- Vanadium（GrapheneOS 內建的強化版 Chromium 瀏覽器）到 9 月 5 日那一版為止累積了四次更新，跟上 Chromium 152 系列。9 月 17 日起換到 153 系列，9 月 19 日那版是 153.0.8010.52.0。
- 核心三條線（6.1、6.6、6.12）都跟上 GKI LTS 的最新修訂。6.6 與 6.12 另外繞過上游 arm64 KVM 的一個錯誤，那個錯誤會讓開了 lockdown 機密模式的裝置開不了機。9 月 17 日那版三條線分別升到 6.1.177、6.6.143、6.12.93，另外新增 6.18 核心分支。
- Pixel 的行動數據機韌體在 9 月 17 日從 Android 17 QPR1 backport 回來，造成部分電信商的相容問題。9 月 19 日再 backport 同一版的 CarrierSettings 修掉，並修好 Wi-Fi 通話元件（wfc-pkt-router）的崩潰。
