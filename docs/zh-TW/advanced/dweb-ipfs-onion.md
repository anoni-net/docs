...
「網站如何發布」這件事，過去十年多了不少選擇。傳統的「自架伺服器 + DNS」之外，CDN 把內容快取到全球節點降低延遲，IPFS 用內容定址讓檔案在多個節點之間流通，Tor Onion 服務則讓網站直接以 .onion 域名運作於 Tor 網路。後兩者常被一起討論，但解決的問題其實不同：IPFS 著重抵抗刪除與審查，Onion 著重連線匿名與管制規避。這篇文章對照兩者的設計差異、真實世界的合用組合，並以 anoni.net 文件站的部署作為實例。
