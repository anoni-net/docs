...
<figure markdown="span">
    <img class="diagram-tall" src="https://assets.anoni.net/diagrams/pq-timeline-v2.zh-TW.svg" alt="後量子密碼遷移時程：從 2023 Signal PQXDH、2024 NIST FIPS 與主流瀏覽器啟用、到 2030–2035 主要系統應完成轉換、2040 舊密碼學硬時間表">
</figure>
...
這節列的主管機關與合規時程都是台灣特定的。香港的對應機關是香港金融管理局（HKMA）與證監會（SFC），病歷等長期資料的保存規範也不同。香港目前尚未查到公開的 PQC 遷移時程。跨地區通用的判斷仍回到 NIST 與 ENISA 的 PQC 遷移時程，先處理 HNDL 風險高的長期資料。
