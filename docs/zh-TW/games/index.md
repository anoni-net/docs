---
title: 互動與呈現
description: 用可以動手玩、或看資料動起來的 3D 小畫面理解隱私與匿名技術。目前三件作品都繞著 Tor：走一遍三跳洋蔥路由的解謎、連線流量在會合點相遇的動態呈現、全球近萬台真實中繼的分布現況。
icon: material/cube-outline
---

# :material-cube-outline: 互動與呈現

這一區放可以動手玩、或單純看著它動的 3D 小畫面，把隱私與匿名技術的概念變成看得到、點得到的東西。全部用 three.js（WebGPU/TSL）在瀏覽器裡跑，免安裝，桌機和手機都能玩。目前三件作品都繞著 Tor，之後會做到其他隱私主題。

## 為什麼做成畫面

隱私技術的保護常常藏在流程裡。以 Tor 為例，封包在三個中繼之間往前走，每經過一跳剝掉一層加密，兩條電路在中間某台中繼相遇後才開始交換內容。發生的順序本身就是重點，文字寫得再仔細，讀的人腦中仍然要自己補上動起來的那一段。畫面把那一段直接演出來。

用對工具往往是一連串取捨，讀過和做過差很多。文件裡寫「三跳要分散在不同 ASN」，讀過就過去了。自己挑三台中繼送出去，看到三跳擠在同一個 ASN 而失敗，下次就會記得為什麼要分散。動手做過一次，留下的印象比讀過十次深。
...
這是互動區的頭三個作品，主題集中在 Tor。隱私裡值得做成畫面的東西還有很多，中繼資料會洩漏什麼、威脅模型怎麼隨處境改變、匿名支付的金流長什麼樣，都在候補名單上。有想法或想一起做，歡迎到[社群](../community/index.md)找我們。
