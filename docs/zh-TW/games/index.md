...
三件作品之中，地球儀整合的資料最多。它把散落在不同機構的公開資料放到同一顆球上，讓「哪裡架得起中繼」與「哪裡連得上 Tor」兩件事得以並置對照。
...
三件作品的原始碼都在 [anoni-net/docs](https://github.com/anoni-net/docs) 這個 repo，位置是 `docs/zh-TW/games/`，三件加起來約四千三百行 JavaScript。沒有建置流程，瀏覽器直接載入 ES module，改完存檔重新整理就看得到。three.js 是本地 vendor 的版本，onion 與 IPFS 版本都不依賴外部 CDN。
...
會 JavaScript 就能改文案、關卡設計與互動邏輯，動到畫面呈現則需要一些 three.js 或 WebGPU 的經驗。產生資料的六支 `gen_*.py` 在 `tools/` 底下，只用 Python 標準庫加 curl。同一個目錄另有一支修正海纜走廊座標的 Node.js 輔助腳本，以及發布資料用的 shell 腳本，都不需要額外安裝套件。
