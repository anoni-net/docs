...
按左側面板上的「開始導覽」，或直接開[帶 `#tour` 的網址](tor-network/play/index.html#tour)，畫面會依序走過七站：全球的中繼分布、台數與共識權重的落差、連線受阻的地方、使用者估計、台灣的位置、台灣那幾層實體設施，最後收在延伸閱讀。空白鍵往下一站，左右鍵來回，Esc 離開。

這一段是為工作坊與教學現場做的。作品連同資料都在[離線閱讀](../offline.md)的自動存檔清單裡，事先開過一次頁面把東西存進裝置，現場沒有網路也開得起來。導覽途中隨時可以自己拖曳地球，按下一站鏡頭就回到腳本上的位置。

某一份資料抓不到的時候，需要它的那一站會整站抽掉，站數跟著少一站。
