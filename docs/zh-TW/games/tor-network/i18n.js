...
const ZH_TW = {
  ...
  pkSub: '裝置容量 <b>{cap}</b> MVA，可靠容量 <b>{rel}</b> MVA，前一年尖峰負載 <b>{load}</b> MVA，也就是可靠容量的 <b>{ratio}%</b>。可靠容量是最大那台主變壓器故障時剩下的還能供多少，超過 100% 代表尖峰時掉一台就撐不住，不是現在過載。',
...
const EN = {
  ...
  pkSub: 'Installed capacity <b>{cap}</b> MVA, reliable capacity <b>{rel}</b> MVA, peak load in the preceding year <b>{load}</b> MVA, which is <b>{ratio}%</b> of reliable capacity. Reliable capacity is what the remaining transformers can carry if the largest one fails, so above 100% means losing one at peak would leave the rest short. It does not mean the site is overloaded now.',
...
const ZH_CN = {
  ...
  pkSub: '装置容量 <b>{cap}</b> MVA，可靠容量 <b>{rel}</b> MVA，前一年尖峰负载 <b>{load}</b> MVA，也就是可靠容量的 <b>{ratio}%</b>。可靠容量是最大那台主变压器故障时剩下的还能供多少，超过 100% 代表尖峰时掉一台就撑不住，不是现在过载。',
