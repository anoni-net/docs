...
const ZH_TW = {
  ...
  gridDist: '分散式光電',
  noteGrid: '長條是各類發電的裝置容量。{plants} 座電廠合計 {cap} MW，資料時間 {stamp} 的淨發電量是 {out} MW。最後一條「分散式光電」{distCap} MW 是全台屋頂與地面型光電的總和，台電那份把它列成一列，沒有個別廠址，所以地圖上看不到它，但它比任何一座電廠都大。地圖上的橘點是電廠，只畫得出 {located} 座，其餘的 OpenStreetMap 沒有位置，不過那 {located} 座已經佔了裝置容量的七成一。細橘線是 {lines} 段 345kV 輸電線，那是電網的骨幹，南電北送走的就是這一層，161kV 以下的配電網完全沒有畫。',
...
const ZH_CN = {
  ...
  gridDist: '分散式光电',
  noteGrid: '长条是各类发电的装置容量。{plants} 座电厂合计 {cap} MW，数据时间 {stamp} 的净发电量是 {out} MW。最后一条「分散式光电」{distCap} MW 是全台屋顶与地面型光电的总和，台电那份把它列成一列，没有个别厂址，所以地图上看不到它，但它比任何一座电厂都大。地图上的橘点是电厂，只画得出 {located} 座，其余的 OpenStreetMap 没有位置，不过那 {located} 座已经占了装置容量的七成一。细橘线是 {lines} 段 345kV 输电线，那是电网的骨干，南电北送走的就是这一层，161kV 以下的配电网完全没有画。',
