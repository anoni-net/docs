...
//   auto      讀者不必管它：圖層清單上沒有按鈕，網址也不記它。由別的機制決定什麼時候
//             打開，目前只有 tw-admin，由各國行政區（admin1）貼近台灣時打開
...
export const LAYERS = [
  ...
  // ── 台灣 ────────────────────────────────────────────────────
  {
    ...
    from: 'docs',
    group: 'tw',
    // 貼近台灣自動出現，跟其他國家的行政區一樣，讀者不必自己開。原本是一顆按鈕，
    // 預設關著，結果其他國家貼近就有界線，台灣反而要去找開關。
    // 仍然是獨立的一層，因為變電所卡片的縣市名稱與導覽的台灣那一站都要它的資料
    auto: true,
