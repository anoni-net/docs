...
const LINE_OP = 0.3;         // onion：電路細線不透明度（additive）
...
function spawnLine(curve, hex) {
  const geo = new THREE.TubeGeometry(curve, 60, 0.028, 5, false);
  const mat = new THREE.MeshBasicNodeMaterial({ color: hex, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false });
  mat.opacity = 0;
