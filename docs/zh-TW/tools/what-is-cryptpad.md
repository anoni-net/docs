...
- **多人共編也維持加密**：當其他人透過你的分享連結加入時，他們在瀏覽器端取得同一把鑰匙，在自己的瀏覽器解密與重新加密所有改動。
...
- **加密碼**：在分享連結之外再加一層密碼，沒密碼者連連結都無法開啟。
- **設定過期時間**：到期後自動失效。
...
切換方式：登入後右上角設定頁可選語言，或於 URL 加 `?lang=zh_Hant`、`?lang=zh_Hans`。舊有以 `zh_CN`、`zh_TW` 為設定值的帳號會自動 fallback 到對應的新語系碼，不會在升級後退回英文。
...
??? question "分享連結被別人轉貼出去怎麼辦？"
    ...
    pad 的密文加密強度沒變，但只要有人取得原始連結（含 `#` 後的鑰匙）就能解開。預防方式：建立 pad 時開「需要密碼」、設定過期時間、用 Matrix 等 E2EE 管道傳連結。若已確認連結外洩，建立新的 pad 把內容複製過去並廢棄舊連結。
