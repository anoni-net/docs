...
LINE、Telegram、WhatsApp 三家在前述三個問題上都有明顯短處，所以不在主名單。它們在台灣社交網路裡仍然不可避免，後面的「在台灣的補充」會說明如何分流。
...
| Session | 隨機 ID（無手機） | 是（衍生自 Signal 協議） | Onion-routed 網路，去中心化 | 限制較多 | 部分 | 是 |
...
- Sealed Sender 隱藏單則訊息的寄件人，不隱藏「這個帳號存在」的事實
...
[LINE](https://line.me/){target="_blank"} 是台灣社交網路的基礎建設，但 E2EE 設計受限。

- **Letter Sealing**（2015 年推出，自 2021 年起預設開啟、無法手動關閉）：覆蓋一對一對話與部分群組訊息。
...
LINE 屬於「有部分加密的社交平台」，稱不上「加密通訊工具」。在台灣切不掉的處境下，分流是務實做法（後面在台灣補充段會展開）。
...
- **威脅模型要對應國安監控**：2019 年後 Telegram、Signal 被大量用於社運動員，《國安法》之後通訊與社群紀錄可成為國安、煽動案件的證據來源，E2EE 工具的政治風險層級比台灣高，脈絡見 [VPN 的風險與選擇](./vpn-guide.md) 的香港小節[^hk]。
...
??? question "Briar 在沒網路時真的能用？"
    ...
    在實體距離內可以。Briar 支援 Bluetooth 與 Wi-Fi Direct 的點對點通訊（約 10–30 公尺，視環境與障礙物而定），雙方直接交換訊息不需要任何 server 或 ISP。長距離不行，要等任一方接上 Tor 後訊息才會跨距離投遞。實際情境：一場活動現場斷網時，能用 Briar 跟同場的人聯絡，但要聯絡場外的人就回不去了。
