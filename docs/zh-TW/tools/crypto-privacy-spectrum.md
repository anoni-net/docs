...
你想匿名訂閱一份政治敏感的電子報、給海外的調查記者打賞、為準備離開的家暴倖存者備一筆對方看不到的資金。每個情境都有人推薦用加密貨幣付款當預設答案，但比特幣與 Monero 的隱私差距，跟現金與信用卡的差距一樣大。
...
公開可查的：BTC、ETH、穩定幣。預設隱藏的：只有 Monero。Zcash 看你如何選用。
...
| Zcash 遮蔽池（z-addr） | 隱藏 | 隱藏 | 遮蔽池內不可見 | 中至高 | 中至高 |
...
剛才那張表的欄位是如何得出的？看一條公開鏈時，外界看得見的東西落在四個維度：

- **帳戶**：誰擁有哪個地址。比特幣、以太坊、穩定幣公開可查，Monero 用 stealth address 隱匿，Zcash 遮蔽交易連地址都看不到。
- **金額**：這筆交易轉了多少。比特幣與多數鏈公開，Monero 用 RingCT 隱藏，Zcash 遮蔽交易完全不可見。
...
- 對手方接受 Monero：用 Monero（取得管道：原子交換 BTC 轉 XMR，即兩種幣不經交易所直接對換），錢包用 Cake Wallet 或 Feather。為什麼選 Monero，看 [§Monero](#monero)。
- 對手方只接受 BTC、USDT：BTC 走閃電網路（Phoenix、Blue Wallet），或用穩定幣搭配每次新建地址。
- 對手方接受預付禮物卡：[為什麼匿名支付重要](../basics/payments-anonymity.md) 討論的低門檻方案可能更簡便。
...
- 願意承擔合規門檻的組織再加 Monero 或 Zcash 遮蔽池作為額外管道，明確標示「進階匿名」。
...
- 場景指引可從 [為什麼匿名支付重要](../basics/payments-anonymity.md) 與 [威脅模型如何建立](../basics/threat-model.md) 延伸。

## 比特幣與以太坊：所有人都查得到

比特幣與以太坊的鏈是公開帳本。只要取得地址，任何人都能查到收發紀錄、餘額、與哪些地址互動過。地址本身沒有名字，但只要在某個 KYC 交易所提領過、留下 IP、寄送給已標記過的地址，鏈上分析公司（Chainalysis、TRM Labs）就能把屬於同一個實體的地址分群（clustering），把地址圖譜跟真實身分連起來。
...
!!! warning "法律風險：混幣服務的執法動向"

    - **Tornado Cash**：2022 年 8 月被美國 OFAC 列入 SDN 清單（首次制裁「智慧合約地址」本身），2024 年 11 月第五巡迴法院裁定制裁逾越授權，OFAC 於 2025 年 3 月除名。開發者刑事訴追另案進行，混幣協調服務的法律風險仍未完全消除。
    - **Samourai Wallet**：2024 年 4 月開發者被美國司法部起訴、服務關閉，2025 年 11 月兩位共同創辦人分別被判 5 年與 4 年有期徒刑。
    - **Wasabi Wallet**：營運方 zkSNACKs 在 2024 年公告停止 CoinJoin 協調服務。

對台灣使用者的判斷重點：技術本身沒有犯罪化，「協調服務」與「混幣輸出」在多數司法管轄會引發合規關切。詳細案例分析見 [倡議組織的匿名捐款管道](../scenarios/nonprofit-anonymous-donation.md) 的 Tornado Cash 段落。社群目前的立場是把這條路線當作背景知識，不再做為主推工具。在香港，除了同樣的合規關切，還要把國安監控擴大後的政治風險層級算進來，與敏感資金流沾邊的行為可能引來額外注意。美國 2025–2026 市場結構立法的整體脈絡，詳見 [台灣 VASP 法 2026](../taiwan/vasp-2026.md) 的國際對照段。
...
!!! note "如何購買 Monero（不靠 KYC 交易所）"

    KYC 交易所自 2020 年起陸續下架 Monero，2022–2024 年間 Binance、Kraken 等主要交易所也相繼跟進。實務上的取得管道：
...
Zcash 的遮蔽交易（shielded transaction）走 zk-SNARKs 路線，鏈上看不到金額、寄件人、收件人。Zcash 同時保留「透明池」（t-addr，跟比特幣一樣公開）給合規情境用，使用者可以選擇要不要走遮蔽。

**真實弱點歷來落在使用率**：遮蔽池的活躍使用者長期遠少於透明池（2024 年以前遮蔽供應量不到一成），匿名集（anonymity set）偏小、分析難度沒有設計上那麼高。不過遮蔽池使用率自 2025 年起明顯成長，實際匿名集大小持續變化，要評估時建議查最新鏈上數據。Halo 2 升級後完全去除可信設定的需要，是密碼學工程的重大進展。
...
- **建議流程**：取得後立即從 t-addr 轉到 z-addr（遮蔽），完成支付後若要兌回法幣再從 z-addr 轉回 t-addr。中間在遮蔽池內的交易能維持高隱私。
...
multisig 把單一私鑰拆成多把鑰匙，需要其中 N 把同意才能花費。常見設定是 2-of-3（三把鑰匙、任兩把可動用）、3-of-5（五把鑰匙、任三把可動用）。
...
- **Zcash**：遮蔽池目前不支援 multisig，透明池可用但失去隱私優勢。

multisig 的常見坑：

- **金鑰備份**：N 把鑰匙都要有可靠的備份策略，否則 N-1 把遺失後資金永遠鎖死。
- **社工攻擊**：攻擊者不必取得所有鑰匙，只要說服 N 個鑰匙持有人簽署一筆惡意交易即可。
...
!!! info "在地脈絡：VASP 法 2026"
    ...
    台灣 VASP 法 2026 施行後，本地交易所對隱私功能的支援會明顯收緊：Monero 與 Zcash 遮蔽交易在本地對接的可能性降低，使用者要更倚賴 DEX、P2P、原子交換等管道。穩定幣發行進入專章管制，個人在自管錢包持有與移轉自有資產，多數情況不落入「經營虛擬資產服務業」的定義。詳見 [台灣 VASP 法 2026](../taiwan/vasp-2026.md)。

!!! info "在地脈絡：香港 VATP 發牌制度（已生效）"

    香港的階段跟台灣不同。台灣的 VASP 法 2026 年 6 月 30 日三讀通過，施行日待行政院指定。香港則自 2023 年 6 月起已實施 VATP（虛擬資產交易平台）發牌制度，法源是《打擊洗錢及恐怖分子資金籌集條例》（AMLO），證監會（SFC）是發牌與監理機關。散戶只能在持牌平台交易符合資格的代幣，Monero、Zcash 這類隱私幣因與 AML/KYC 要求衝突，實務上幾乎無法在持牌平台上架。也就是說，香港的收緊已經運作數年，台灣則在等施行日[^hk]。
