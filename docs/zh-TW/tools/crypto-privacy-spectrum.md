...
比特幣與以太坊的鏈是公開帳本。只要拿到地址，任何人都能查到收發紀錄、餘額、與哪些地址互動過。地址本身沒有名字，但只要在某個 KYC 交易所提領過、留下 IP、寄送給已標記過的地址，鏈上分析公司（Chainalysis、TRM Labs）就能把屬於同一個實體的地址分群（clustering），把地址圖譜跟真實身分連起來。
