...
上面三家的分野（消費版可能訓練、企業版預設不訓練）是境外服務的通則。在中國境內提供的生成式 AI 服務，例如 DeepSeek、豆包、Kimi、通義這些，適用的是另一套規則，前面的判斷方式不能直接套用。

《生成式人工智慧服務管理暫行辦法》2023 年 8 月 15 日施行，由網信辦會同六個部門發布[^cn-genai]。跟工作使用有關的有三點：

- **使用者要實名**。服務提供者須驗證使用者身分，你在境內 AI 上的提問不是匿名的
- **輸入與輸出都要留存**。法規要求保存使用者輸入與生成內容的紀錄，這跟境外服務「你可以關掉訓練」的設定是不同性質的事，那是產品選項，這是法定義務
- **具輿論屬性或社會動員能力的服務要做安全評估與演算法備案**，內容審核內建在服務裡

實務上的判斷因此不同：

- 境外服務要問的是「會不會拿去訓練、保留多久」。境內服務要先問的是「這段內容我願不願意讓它跟我的實名綁在一起、被保存下來」
- 企業版與資料處理協議在境外能解決供應商訓練的問題，在境內解決不了法定留存與可調取
- 敏感工作內容不適合放進境內服務，本地模型或自架在這個處境下的價值比在境外高得多

同一套判斷也適用於境內平台附帶的 AI 功能，例如辦公軟體、輸入法、瀏覽器裡的助手。
...
前面幾節處理你自己複製貼上的內容。實際上更常見的洩漏來自你沒有意識到正在送出的那些：

- **瀏覽器的 AI 外掛**：多數需要讀取當前頁面才能運作，你打開的內部系統、客戶資料、後台頁面都在範圍內
- **編輯器與 IDE 的 AI 助手**：預設會把周邊程式碼甚至整個專案當成脈絡送出，裡面可能有金鑰、內部主機名、客戶名稱
- **會議轉錄與摘要工具**：一旦開啟就錄下整場對話，包含你沒預期會被記錄的閒聊，以及在場其他人沒有同意被錄的部分
- **輸入法的 AI 功能**：雲端聯想與改寫會把你打的字送到伺服器
- **辦公軟體內建的助手**：文件、郵件、試算表的內容成為預設輸入

處理方式跟前面不同，這裡要看的是設定而不是習慣：安裝前確認它讀什麼、在哪些網站或專案停用、以及開會前先問在場的人。組織要處理的話，這一類工具比員工手動貼上更值得先盤點。
