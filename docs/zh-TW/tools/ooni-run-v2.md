...
社群維運的 OONI Run 連結 ID 是 `10238`，網址 [run.ooni.org/v2/10238](https://run.ooni.org/v2/10238){target="_blank"}，目前納入 anoni.net 的官網、Cryptpad、Etherpad、SearXNG、Send、Matrix 與 docs 站。協助者用 OONI Probe 安裝這條連結後，每次跑測試都會把這幾個自架服務的可達性回傳到 OONI 公開資料庫。對社群來說，這是長期確認「我們的服務在台灣不同電信商眼中還連得上」的低成本方式。
...
要更系統地評估這些風險，可參考 [威脅模型怎麼想](../basics/threat-model.md)。
