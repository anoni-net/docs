...
清單裡的 `search.anoni.net` 跟其他網址走不同架構。它直接指向社群自架線路上的主機，用浮動 IP 搭配 DDNS，其他網址前面都有 CDN。浮動 IP 更換的空窗期內 DNS 還指著舊位址，所有協助者在那段時間都會測到連線逾時，OONI 記成 `blocking: tcp_ip`。看到那一項顯示異常，先想到線路波動。確認是否為封鎖，要看同一時段其他 ASN 有沒有一起出現。留著它是刻意的，清單裡只有它沒有 CDN 前置，剛好能跟其他九個網址對照。
...
你可以挑選圖標、顏色來自訂連結外觀，並填寫頁面中必要的資料欄位。
...
在建立頁面完成後取得的 <https://run.ooni.org/v2/10328>{target="_blank"} 連結可以直接分享給協助者，打開後會看到一個簡單的介紹頁面。如果檢測者沒有安裝 OONI Probe，頁面也會引導他們前往應用程式商店下載、安裝。
...
??? question "可以開著 VPN 執行 OONI Probe 嗎？"
    ...
    不建議在開啟 VPN 的情況下執行 OONI Probe。開 VPN 測試時，測到的是 VPN 提供者所在地的網路環境，反映不出協助者本地的封鎖狀況。執行 OONI Probe 測試前請先關閉 VPN 或其他翻牆軟體。
