...
- **免費 VPN 的商業模式**：免費 VPN 要從別處賺錢。2016 年一份分析 Google Play 上 283 個 VPN App 的學術研究發現，38% 被檢出含某種惡意程式（含 adware、間諜軟體等）、18% 完全不加密流量、84% 會洩漏使用者流量，且多數內嵌第三方追蹤函式庫[^free-vpn]。這份研究是 2016 年的資料，但免費 VPN 靠使用者資料或廣告獲利的商業模式，至今沒有改變。要免費又可信，看後面非營利導向的選項。
...
[Mullvad](https://mullvad.net/){target="_blank"} 在隱私設計上做得最徹底。註冊不需要 email，只給你一組隨機帳號編號。付款接受現金郵寄、Monero、Bitcoin，不需要把身分綁進去。2023 年 9 月完成全伺服器 RAM-only 遷移，基礎設施與 app 多次經 Cure53 稽核、Web App 由 Assured 獨立稽核，客戶端開源。前面提到的瑞典警方搜索案例，就是 Mullvad no-log 政策的實證。
...
- **澳門**：VPN 合法、無系統性封鎖，屬於受規範。《網絡安全法》（第 13/2019 號）要求電信實名，ISP 須留存可把連線追溯到具體用戶的對應紀錄至少一年[^macau]。連線本身會被長期記錄，敏感任務要把這點算進去。
