...
Tor Project 自己也做了一個叫 Tor VPN 的 app。它不是商業 VPN，做的是把 Tor 包成 VPN 的操作介面，用系統的 VPN 機制把你選定的 app 流量導進 Tor 網路，底層引擎是 Arti（Rust 版的 Tor）加上 Onionmasq[^torvpn-about]。免費、開源（BSD 3-Clause），目前只有 Android（7.0 以上），iOS 與桌面版都還沒有[^torvpn-install]。

它的意義是把這整頁的核心取捨變成一個能直接裝來用的選項。一般 VPN 把信任集中到一家業者，Tor VPN 的出口是 Tor 的志願者中繼，沒有任何單一方同時看得到你的來源與目的地，每個 app 還各走一條獨立的 Tor circuit、拿到不同的出口 IP，降低跨 app 被關聯的機會[^torvpn-intro]。它走的是 Tor 的分散式信任，省掉了單一業者這個信任單點。
...
- 目前只有 Android。要在 Linux 桌面做類似的全流量 Tor 隔離，可看社群介紹的 [oniux 核心層級隔離工具](../blog/posts/oniux-kernel-level-tor.md)。
