...
- **加密貨幣錢包的助記詞：**建立錢包時它要你抄下、收好的那 12 或 24 個英文字，就是密語最大規模的應用，把錢包的金鑰變成一串你抄得下來的字（這套標準叫 BIP-39，連繁體中文詞表都有）[^bip39]。背景可參考 [加密貨幣的隱私光譜](./crypto-privacy-spectrum.md)。
...
- **那張著名的漫畫：**xkcd 的「correct horse battery staple」畫的就是同一件事，四個隨機英文字串起來，電腦難猜、人卻好記[^xkcd]。
...
選字有兩個刻意的限制。第一，外來語一律收已經進英文字典、寫成一個單字的詞，不收 `feng shui`、`kung fu`、`dim sum` 這類中間有空格的詞組。第二，我們不自己音譯華語，因為台灣同時有漢語拼音、威妥瑪、通用拼音幾套標準，香港另有粵語拼音（Jyutping）等系統，拼法會打架，所以只收英文拼法已被字典固定下來的字。

亞洲外來語只占約 3.8%，一句六字密語大約每五組才會出現一組帶亞洲字，其餘都是常見英文字。這是刻意的設計，安全性與 EFF 原版完全一致，亞洲味只是多一點親切感，不必期待每次都抽到 `tofu`、`oolong`。

??? note "為什麼亞洲外來語只占約 3.8%"
    像啤酒的酒精濃度（ABV）一樣，3.8% 是刻意挑過的比例（7776 字中 292 個）。上限取決於英語實際吸收成「單一字典詞」的可辨識亞洲外來語有多少。把 OED、Merriam-Webster、Cambridge 查過一輪，台灣、香港與華語圈讀者認得出來的大約 330 個，其中 292 個納入清單，另外約 40 個留作備援，可辨識的外來語幾乎就到頂了。

    想再往上拉（例如 10%、約 778 個字），只會逼出兩種做法，兩種我們都不採用：

    - **拿冷僻詞充數**（`puttee`、`howdah`、`nilgai`、`maund`），多數人拼不出、唸不準、也記不住。冷僻詞會破壞這份清單要守住的 EFF 特性，也就是抄得下來、唸得回去、不容易出錯。
    - **改用羅馬拼音的官話或注音音節**，那屬於另一個專案。台灣同時有漢語拼音、威妥瑪、通用拼音幾套拼法，香港另有粵語拼音（Jyutping），彼此打架、容易產生歧義。

    比例拉高，密語並不會更強。無論抽到 `tofu` 還是 `the`，每個字增加的猜測難度都一樣，強度只跟詞表恰好 7776 字、每次擲骰機率均勻有關，跟字的來源無關。所以跟啤酒不同，這裡「濃度更高」不會多給你什麼。文化覆蓋和好用性衝突時，好用性優先。
...
選字的完整過程、292 個字的來源分布，以及建置流程擋掉了什麼，見社群的 [一份英文密語詞表裡的 292 個亞洲外來語，是怎麼選出來的](../blog/posts/asian-diceware.md)。
...
**方法一，用實體骰子查表：**擲 5 顆骰，由左到右讀成一個五位數（每顆 1 到 6）。那張表是照代碼從小到大排列的，翻到、或用搜尋找到開頭是你這五位數的那一行，後面接的字就是抽到的字。重複 6 次，把 6 個字用 `-` 接起來。例如擲出 `6 3 4 4 4`，讀成 `63444`，查到 `tofu`。六次擲完就得到一句像 `tofu-oolong-gecko-mango-bazaar-haiku` 的密語，讀得出來、抄得下來，比一串亂碼好記得多。
...
- **要自己手動輸入的密碼：**家裡 Wi-Fi 密碼、電視或遊戲主機上登入串流帳號、與家人共用的裝置，都得一個字一個字敲進去，一串亂碼很難輸入，密語好輸入又好唸。
- **加密磁碟、備份與金鑰的口令：**全碟加密、加密備份、PGP 私鑰這類「打錯就解不開、外洩就全暴露」的口令，適合用密語。
- **把號碼唸成字、不會聽錯：**要核對金鑰指紋、安全碼這類一長串數字時，先用骰子表把號碼換算成字，唸字比唸數字不容易出錯。PGP word list 用的就是同一招[^pgpwords]。
- **和信任的人約定暗號：**把詞表當成只有你們懂的暗號本，用來確認身分或傳遞約定好的訊號，展開見下方〈和信任的人約定一組暗號本〉。
...
- **一定用真亂數：**實體骰子最簡單直接。若改用程式產生，請走密碼學安全的方法（見上面〈給會寫程式的人〉），不要自己挑字，也不要挑「看起來很隨機」的字。人挑的字猜起來容易得多。
...
想提升日常密碼的強度，讀到上一節就夠了。以下是這份詞表的進階用法，記者、公民團體、需要低調傳遞訊號的人可能特別用得上。
...
社群參加工作坊、小聚、研討會這類實體活動時，也會帶幾本印好的小冊到現場發送。如果你在活動裡取得一本、或剛好遇到我們，非常歡迎過來聊聊，問密語、問匿名網路，或只是打聲招呼都可以。想知道我們最近會出現在哪，見 [活動參與](../activity/index.md)。想直接找我們，可以到社群的 Matrix（入口見 [社群自架服務](../community/tools.md)），或 [持續關注](../contact.md) 我們的後續消息。
...
!!! info "還在規劃：一個匿名服務平台"
    社群接下來也想做一個類似 [AnonTicket](https://anonticket.torproject.org/){target="_blank"} 的匿名服務平台（還在規劃階段）。AnonTicket 是 Tor Project 的匿名服務，讓人不必透露身分就能匿名向 Tor 的 GitLab 提交問題回報，用系統產生的一組隨機英文字代碼辨識身分，而非 email。這種帳號代碼正是從隨機詞表抽字組成的，asian-diceware 詞表就是為了當未來同類服務的代碼字源而準備。AnonTicket 本身如何用在與 Tor 上游協作，見 [Tor Project 生態與對接](../community/tor-project-ecosystem.md)。
