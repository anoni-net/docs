...
多數人一開始就想切太細，層數越多，每天要記得的規則越多，撐不過三個月就會開始便宜行事，然後在某次疲勞的時候用錯帳號。先回到 [威脅模型如何建立](./threat-model.md) 的三個問題，再決定層數。
...
長期觀察一個帳號的活躍時段，能推出所在時區、上下班時間、週末習慣。兩層如果同步在深夜安靜、早上八點活躍，作息模式本身就是識別特徵。出國時兩層同時跳到新時區更明顯。

處理方式是接受這件事擋不掉，改成減少可觀察的樣本：次要層降低發文頻率、用排程工具錯開發布時間、旅行期間直接讓次要層安靜。
