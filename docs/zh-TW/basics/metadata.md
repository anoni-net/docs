...
近年 Encrypted Client Hello（ECH，把 TLS 握手裡的目標網域名也一起加密的機制）開始改變 SNI 這一塊。Firefox 119、Chrome 117 之後預設支援，Cloudflare 等大型 CDN 也預設開啟，SNI 可以不再以明文出現。但它需要伺服器端支援、在加密 DNS（DoH）環境下效果最好，也可能被中間設備阻擋，所以還不能假設每條連線的 SNI 都被保護。
