...
Google 在 2019 年說明過落差，使用者無法清除指紋，因此無法控制自己的資訊怎麼被收集，「我們認為這破壞了使用者的選擇權，是錯的」[^google2019]。同一家公司後來的政策走向見下方〈各家瀏覽器現在做到哪裡〉。
...
成立條件是覆蓋範圍要完整。漏掉的欄位會變成穩定的錨點，讓隨機化過的部分失去意義。
...
**Firefox** 在 `145` 版補上第二階段防護。已知的指紋腳本沿用加強型追蹤保護（Enhanced Tracking Protection）的清單封鎖，未列名的則改用限制 API 輸出的方式處理，兩項都在無痕視窗與加強型追蹤保護的嚴格模式預設開啟。Mozilla 表示被判定為唯一的使用者比例因此接近減半，全域預設開啟仍在進行中[^mozilla]。另外一個開關 `privacy.resistFingerprinting` 沿用 Tor Browser 的一致化路線，預設關閉，需要自行到 `about:config` 開啟。
...
政策方向另有一項變化值得記住。Google 在 2024 年 12 月 18 日公告修改廣告平台政策，移除禁止使用裝置指紋的條款，2025 年 2 月 16 日生效[^policy]。英國資訊委員辦公室（Information Commissioner's Office, ICO）隔日回應，稱該決定不負責任，並指出使用指紋識別的業者仍須證明符合資料保護法在透明、同意與可刪除等方面的要求[^ico]。
...
讀結果的時候有兩件事要記得。測試站的樣本來自主動造訪的人，判定為唯一的比例會偏高。真正有用的是逐項看哪幾個欄位識別力最高，那些欄位是你調整之後收益最大的地方。
...
擋不掉的部分：你主動登入的帳號。指紋防護處理的是不具名的跨站關聯，登入行為本身直接告訴網站你是誰。
...
擋不掉你自願交出去的資訊。換裝置與換工具改變的是關聯，你在單一網站上留下的內容沒有因此減少。
