...
summary: "CryptPad 是少數讓伺服器看不到內容的線上協作工具，但介面過去缺少正體中文。anoni.net 社群花兩年半在 CryptPad 上游做翻譯，2026/05 隨 CryptPad 2026.5.0 上線，希望讓那些用得上加密協作工具、卻被語言擋在門外的正體中文使用者，無論在台灣、香港、澳門或其他華語環境，能因為這道門檻消失而把它放進日常選項。"
description: "CryptPad 是少數讓伺服器看不到內容的線上協作工具，但介面過去缺少正體中文。anoni.net 社群花兩年半在 CryptPad 上游做翻譯，2026/05 隨 CryptPad 2026.5.0 上線，希望讓那些用得上加密協作工具、卻被語言擋在門外的正體中文使用者，無論在台灣、香港、澳門或其他華語環境，能因為這道門檻消失而把它放進日常選項。"
---
...
在台灣，想找一套真正不會被第三方平台默默收走內容的協作工具，其實沒有想像中容易。Google Docs、Notion、Microsoft 365 都很好用，但每一段文字、每一個改動，都會以可被服務商讀取的形式存放在他們的伺服器上。在這之上，演算法、廣告、訓練語料、政府調閱請求，各有各的取用方式。對多數人來說，這層差異感覺不出來。對另一群人，例如記者寫不能曝光的稿、社運工作者協商不能被監聽的策略、NGO 整理脆弱使用者的求助紀錄、學者研究敏感議題，這層差異往往決定一份草稿能不能安全寫得出來。
...
這套工具過去有一個明顯的門檻，介面只有英文與簡體中文，正體中文是缺的。從 2023 年底在 CryptPad 上游開的第一個 PR 起算，到 2026 年 5 月 13 日 [CryptPad 2026.5.0「🌷 Spring release」](https://github.com/cryptpad/cryptpad/releases/tag/2026.5.0){target="_blank"} 正式把正體中文（zh_Hant）收進內建語系，前後花了兩年半。社群自架的 [cryptpad.anoni.net](https://cryptpad.anoni.net/){target="_blank"} 升級完成，現在不用懂英文也能完整使用這套加密協作平台。**我們投入這次翻譯的初衷，就是希望讓那些用得上加密協作工具、卻被語言擋在門外的正體中文使用者，無論在台灣、香港、澳門或其他華語環境，能因為這道門檻消失而把它放進日常選項。**

<!-- more -->

## CryptPad 是什麼

把 CryptPad 的基本資料先擺出來。它由法國 [XWiki SAS](https://xwiki.com/){target="_blank"} 開發，使用 [AGPLv3](https://github.com/cryptpad/cryptpad/blob/main/LICENSE){target="_blank"} 授權，定位是**端對端加密（E2EE）的線上協作辦公套件**。一個帳號可以使用以下應用：
...
既然要花兩年半把介面翻成正體中文，那就得先說清楚為什麼這套工具值得這個工作量。社群自架的服務不只 CryptPad，也有 [Etherpad](https://pad.anoni.net/){target="_blank"} 做即時共筆、Matrix 做即時討論（三者分工見 [社群自架服務](../../community/tools.md)）。CryptPad 在我們的選擇順位裡，承擔的是「需要長期保存、需要加密、需要多人協作完整文件」的場景。

我們選擇它，也願意把它推給社群裡的其他人，理由有以下幾點：
...
**開源、可自架、可審視**：AGPLv3 授權保證任何衍生服務都必須開源，我們自架時也能完整檢查程式碼。加密協議與資料結構公開可審視，跟 Tor、Tails、OONI 一樣，是「可被驗證的隱私」。

**規則由維運者與社群決定**：自架的好處跟 [自架 Matrix](2026-discord-matrix-statement.md) 的理由一致，紀錄保存策略、註冊政策、頻道規則由我們決定，可預期、可問責、可隨社群需求調整。

**有歐洲公部門與民間組織的真實使用驗證**：CryptPad 在歐洲多個公部門專案、NGO 與研究單位內被採用，代表它在合規、可靠性、長期維護這三個面向上都經過真實使用壓力，把它推給更多正體中文使用者時不必擔心是「demo 漂亮的玩具」。

## 正體中文（zh_Hant）翻譯工作的進展

翻譯一套線上協作工具的介面，不像翻一篇文章。CryptPad 主應用、Accounts plugin 與 User Guide 加起來上千條字串，每一條都要對齊它在介面上會出現的情境、顧及上下文、避免在「儲存」「另存新檔」這類詞之間混用。再加上 CryptPad 還在持續開發，新功能會帶來新字串，每個版本上線前都得回頭再校一輪。這件事從 2023 年底開了第一個 PR 起步，一路走到 2026 年 5 月把 zh_Hant 收進內建語系，前後橫跨兩年半。把時間線整理出來：
...
這次合併除了把 `zh_Hant` 與 `zh_Hans` 打開為正式語系，也加上了 locale alias 機制，讓舊有以 `zh_CN`、`zh_TW` 為設定值的帳號可以自動 fallback 對應到新的 `zh_Hans`、`zh_Hant`，不會在升級後跑回英文。對既有使用者來說，這次升級是無痛的。對還沒用過 CryptPad 的正體中文使用者來說，門檻變低了一格，這就是這兩年半最具體的成果。

## 關於「正體中文」與「繁體中文」的用字

CryptPad 介面選單最早寫的是「中文(繁體)」。我們在 PR #1329 提的修改是改成「中文(正體)」。改字面看起來是小事，背後是社群比較傾向「正體」這個用詞。「繁」字暗示「相對於簡體比較複雜」，但這套字系在台灣、香港、澳門的使用脈絡，本來就是延續漢字源流而來的正統寫法，沒有「繁」與「簡」的對比關係。OS 與多數軟體仍寫「繁體中文」是事實，我們不要求所有人都改，但在自己貢獻的翻譯裡能改的時候就改。用什麼字稱呼一個族群使用的字系，是普及工作的一部分。

## 怎麼開始用 cryptpad.anoni.net

如果讀到這裡你覺得這套工具有機會放進日常選項，下面是上手方式：
