...
??? question "有三個編號的寫法與其他章節不一樣"

    報告全文出現 39 個 `GTG` 編號，其中 36 個用連字號。只有 `GTG 16005`（第 147 頁）、`GTG 16012` 與 `GTG 16003`（第 152 頁）這三個改用空格，而且全部出現在本章的小標題裡，其餘章節的標題一律用連字號。那是報告排版上的疏漏，本文照原文抄錄。用 PDF 搜尋這三個編號時要記得改用空格，打連字號會找不到。
