...
!!! tip "在台灣，你也可以幫上忙"
    ...
    手上有一台 VPS（雲端主機）或實體主機，再加一個網域，就能架一個 Tor WebTunnel 橋接，讓被審查切斷的人能連回外面的網路。架不了伺服器也沒關係，打開瀏覽器執行 [Snowflake](../../tools/tor-snowflake.md) 一樣能貢獻匿名流量。
...
封網期間，伊朗只剩一個被高度過濾的國內網路可用，銀行、外送等本地服務還能運作，但對外的連線幾乎全斷。NetBlocks 把伊朗的封網列為現代史上最長的全國性斷網，全國 9000 萬人口裡，大多數在近三個月幾乎連不上國際網路[^3]。

## 社群的 WebTunnel 在伊朗重新開放網路後流量跳升

社群在台灣架設的 Tor WebTunnel 橋接，平常就在背景持續運作，幫連不上 Tor 的人繞過審查連進來。伊朗重新開放的那兩天，這個節點的連線明顯跳升，流量比平常高出許多。看到流量回來的當下，我們鬆了一口氣，那代表又有人能重新連上外面的網路。
...
封網期間，當地人連 Tor 都連不上，因為整條對外連線都斷了。連線一恢復，很多人急著想知道外面發生了什麼、跟失聯的親友重新聯絡上。記者要把當地消息傳出去，公民團體需要和外界協調，都得連上那些被長期封鎖的網站和服務。要繞過審查，多半需仰賴 Tor，而在 Tor 本身也被封鎖的地方，還得透過各地志工架設的橋接才連得上。他們透過橋接連上 Tor 時，有一部分連線就經過社群架在台灣的節點（社群在新加坡也架了一台，當時沒有被分配到流量）。
...
流量並沒有在那兩天之後就退去。之後幾天，連線仍持續經過橋接。根據 NetBlocks 等監測機構，伊朗的恢復並不完整，行動網路一度仍中斷、家用 Wi-Fi 才先恢復，主要社群平台的封鎖也還在，甚至比封網前更嚴，當地人要連到外面的一般網站，多半需仰賴 VPN 之類的工具才連得出去[^3][^4]。對很多人來說，就算網路「重新開放」，要連到外面還是得繞過大量封鎖，而社群的 WebTunnel 就是其中一種繞過封鎖的方式。

流量持續湧進社群架設的 WebTunnel，現有的幾個節點顯然不夠用。所以我們想邀請更多有能力的人一起架設橋接，讓更多需要連到外面的人連得上。
...
WebTunnel 的做法不同，它把 Tor 流量包進一個真正的 HTTPS 連線裡（就是平常瀏覽器網址列上鎖、開頭 https 的那種安全連線）。在審查者眼中，連到 WebTunnel 橋接跟連到一個普通網站沒有兩樣。要封鎖它，就得連帶封掉大量正常的 HTTPS 網站，代價高到審查者通常下不了手。WebTunnel 因此成為最難被封鎖的橋接之一，在中國、俄羅斯都已經實際派上用場。
...
如果沒辦法協助架設 WebTunnel，也可以打開瀏覽器執行 [Snowflake](../../tools/tor-snowflake.md)，開著分頁就能貢獻匿名流量，幫連不上 Tor 的人繞過審查。等你準備好投入一台伺服器，再回來架 WebTunnel。
