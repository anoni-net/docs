...
三輪最重要的是都用同一支手機、同一個門號，而且每一輪都在行動網路上完成。位置能固定會讓資料更好比對，中途需要移動也不影響參與，只要確認沒有切回 Wi-Fi 就好。
...
**部分驗證官方的說法。** 公告稱語音、簡訊、細胞廣播正常，只有高流量服務受影響。三份公告對受影響範圍的描述並不一致，台灣大哥大的公告把 LINE、WhatsApp、M+ 等通訊軟體列進會出現連線延遲與不穩定的範圍，行政院的版本則是文字傳輸正常。`ndt` 與 `dash` 只能檢查高流量那一側，語音、簡訊與細胞廣播不在這兩個測項的能力範圍內。可驗證的是界線的其中一邊，仍然值得做。公民社會有能力自己驗證公共措施的實際效果，本身就是韌性的一部分。
