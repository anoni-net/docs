...
流通中的唯一一個數字來自[外電報導](https://therecord.media/taiwan-mobile-5g-speed-reductions-han-kuang){target="_blank"}，引述政府規劃文件指出目標是降到正常速度的約 1%。這個數字未經官方證實，也沒有出現在任何一份公告裡。週四那 30 分鐘實際掉到什麼程度，目前沒有可查證的公開紀錄。
...
最常見的誤解是把異常（anomaly）當成封鎖。OONI 的 `anomaly` 只代表測試未照預期完成，成因包含審查、網路不穩、ISP 暫時故障，以及測試程式本身的問題，將 `anomaly` 比率直接視為封鎖率會產生假指控。以 `tor` 測項近 30 天的實測為例，加拿大 `21.2%`、瑞士 `22.2%`、紐西蘭 `41.1%`，皆為沒有審查的國家。中段數值屬於雜訊，只有比率極端高的少數國家與現實相符。
