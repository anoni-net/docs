...
!!! note "翻譯說明"
    ...
    我們目前還沒有針對「**Bridge 橋接類型**」作專門的介紹，橋接節點主要是協助無法直接連入 Tor 的洋蔥路由中，透過橋接節點介接連入，目前有一個最快的方式建立橋接點，是透過偽裝成**線上視訊**的連線方式協助轉接，僅使用**瀏覽器**就可以協助建立，詳細或試用可以參考「[Tor Snowflake 橋接點建立（網頁版）](../../tools/tor-snowflake.md){target="_blank"}」。
