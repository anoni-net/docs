...
- **Measurements API**：`https://api.ooni.org/api/v1/measurements`，接受精確到秒的時間區間，回傳逐筆紀錄。本文的主要資料出自此端點。
...
**當天逐筆量測**用 measurements 端點，`probe_cc=TW`、`since=2026-08-12T16:00:00Z`、`until=2026-08-13T16:00:00Z`，對應台灣時間 8 月 13 日整天。`test_name` 分別為 `ndt`、`dash`、`web_connectivity`、`tor`、`psiphon`。降速時段等時段統計另以 UTC `06:00` 到 `07:30` 的區間查詢。
