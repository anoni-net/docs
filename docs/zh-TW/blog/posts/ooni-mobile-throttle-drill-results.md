...
summary: "台灣長期沒有大規模的網路降速或斷網，OONI 公開資料庫裡台灣的紀錄一直是正常運作的網路。8 月 13 日北部七縣市降速 30 分鐘，社群號召大家用 OONI Probe 記錄，當天全台留下 238 筆連線速度測試與 235 筆影音串流測試，行動網路上完成的來自 47 支裝置。前 30 天行動網路的 564 筆量測裡，下載低於 2,000 kbit/s 的只有 4 筆。降速時段有一支中華電信行動的手機量到 788 到 1,709 kbit/s，是台灣資料裡沒有出現過的樣態，但僅為單一裝置的 6 筆紀錄，不足以推論全國。本文記錄的是網路狀態，不評斷業者是否確實執行指令。內容包含降速在數值上的樣態、使用時的感受如何對應到欄位、降速與斷網在 OONI 資料上的差別、每個結論如何從公開 API 一步步得出，以及超出本批資料範圍的問題。最後介紹 OONI 公開資料的四個入口，歡迎一起投入研究。"
description: "台灣長期沒有大規模的網路降速或斷網，OONI 公開資料庫裡台灣的紀錄一直是正常運作的網路。8 月 13 日北部七縣市降速 30 分鐘，社群號召大家用 OONI Probe 記錄，當天全台留下 238 筆連線速度測試與 235 筆影音串流測試，行動網路上完成的來自 47 支裝置。前 30 天行動網路的 564 筆量測裡，下載低於 2,000 kbit/s 的只有 4 筆。降速時段有一支中華電信行動的手機量到 788 到 1,709 kbit/s，是台灣資料裡沒有出現過的樣態，但僅為單一裝置的 6 筆紀錄，不足以推論全國。本文記錄的是網路狀態，不評斷業者是否確實執行指令。內容包含降速在數值上的樣態、使用時的感受如何對應到欄位、降速與斷網在 OONI 資料上的差別、每個結論如何從公開 API 一步步得出，以及超出本批資料範圍的問題。最後介紹 OONI 公開資料的四個入口，歡迎一起投入研究。"
---
