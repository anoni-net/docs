...
!!! info "台灣專案與社群的現況"
    ...
    在台灣，多數專案或社群已經有「提案、討論、共識」的流程，只是**不一定用 ADR 這個名字**，也較少把決策文件以 `adr/` 目錄或 MADR 模板的形式公開出來。
    ...
    - **企業開源專案（台灣團隊）**：  
      某些團隊在公司內部有用「決策紀錄」或「設計文件」的方式管理架構變更，但經常只在內部維運知識庫中存在，開源出去的只有程式碼本身，看不到決策歷史。
    ...
    - 新手或外來的人看 ADR，就能很快懂「**原來系統當初是為了什麼才做成現在的樣子**」，也比較不會反覆重來同樣的討論。
