...
atlas.js 後來替東亞另外畫了一張 2048 見方的細部貼圖，那一塊每度有 37 個像素，台灣本島
變成 52 像素寬。結論沒有改變：台北市 272 平方公里在那張上面只有 36 個像素，縣市界線
仍然要走向量線。細部貼圖負責的是陸地填色的形狀，界線仍然走向量。
...
MUST_KEEP = {
    ...
    # 這兩座才是最脆弱的。實測太平島的環只跨 0.0121 度、東沙島 0.0243 度，都比上面
    # 六座裡最小的小琉球（0.0353 度）還小，MIN_SPAN 只要往上調一點就會先濾掉它們，
    # 而且是靜默的：上面六個斷言照樣全過，輸出的緯度範圍卻悄悄從 10.37N 縮上去。
    # 檔頭本來就聲稱這兩座有測試守著，先前漏放在這裡。
    "東沙島": (20.70, 116.72), "太平島": (10.37, 114.36),
}
