...
PROSE_RULES = [
    ...
    # 同一個句型把「而」省掉，靠逗號銜接。上面兩條只認字面的「而是」，這個變體
    # 整個漏掉，全站累積了 16 處，其中幾處還是翻譯版本比 zh-TW 落後留下來的。
    ("bushi-doushi", ERROR, re.compile(r"不是[^。！？\n]{0,30}?，\s*(?:是|就是)[^。！？\n]"),
     "禁用「不是…，是…」句型（「不是…而是…」省略「而」的寫法），改用正向直述"),
