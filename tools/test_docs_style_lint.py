...
CASES: list[tuple[str, bool, str]] = [
    ...
    ("我們自己卻收精確值，說不過去。", True, "口語「說不過去」"),
    ("我们自己却收精确值，说不过去。", True, "簡體的「说不过去」一樣要攔"),
    ("量了只會把平均值灌爆。", True, "誇飾「灌爆」"),
    ("我們自己卻收精確值，前後並不一致。", False, "改成書面語就不該攔"),
    ...
    ("透過公開網關讀取。", True, "「網關」是中國慣用詞"),
    ("透過公開閘道讀取。", False, "臺灣用「閘道」"),
    ("涉及「安全網關」產品的採購。", False, "照錄合約裡的產品名稱"),
    # 這幾個臺灣也用，刻意沒收進規則
