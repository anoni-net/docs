...
# 東亞核心。中國東南沿海、台灣、日本、韓國、菲律賓、中南半島，也就是讀者最常
# 放大去看的那一塊。範圍收得比整個亞洲小，省下來的額度才夠讓這一塊真的細。
EAST_BOX = (105.0, 0.0, 150.0, 50.0)

# 兩份檔案的用途不同，容差分開給。
#
# 海岸線是讀者實際看到的輪廓，方塊感就是從這裡來的，所以東亞那一塊留到 0.002 度
# （約 220 公尺），量出來的點距中位 2.23 公里，貼近時在螢幕上不到 6 個像素。
#
# 國界只有兩個用途：判六角格的國碼，還有畫成底圖貼圖。貼圖是 2048 × 1024，
# 一個像素就是 19 公里，再細也畫不出來；六角格最細那一級是 4.3 公里，5.5 公里的
# 點距夠用了。所以這一份可以粗一個量級，省下 180 KB。
#
# 三個數字分別是世界其他、其餘亞洲、東亞核心。
COAST_TOL = (0.25, 0.03, 0.002)
COAST_ROUND = (2, 2, 3)
BORDER_TOL = (0.25, 0.05, 0.01)
BORDER_ROUND = (2, 2, 2)
