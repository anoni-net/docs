...
並行連線數預設 3 條。2026-09-25 iPhone 15 Pro 對 Mac Chrome 154 的實測，iPhone 送出 100 MB 時，1 條每秒 4.8 MB、3 條 16.2 到 17.4 MB，4 通道 · 1 連線是 4.6 MB。Mac 送出時單條就有每秒 14 到 26 MB（20 MB），3 條 · 4 通道送 100 MB 是每秒 51 到 53 MB。選中的候選對另外記下兩端位址的類別（`localNet`、`remoteNet`），分辨流量走區網還是 Tailscale。Chrome 在沒有相機權限時不給本機位址，遠端的 mDNS 與 prflx 候選常常也不給，這些情況記成 `unknown`。所以回送檢查裡是 `unknown`，接假攝影機、已授權相機的 `run-qr-camera.mjs` 裡才分得出 `lan`。
