...
| 檔案 | 走哪一條路 |
|---|---|
| `run-loopback.mjs` | 複製貼上。一顆 Chrome 開十個分頁，走經由介紹連上第三台、兩台同時互掃、同一張 QR code 被兩台掃到、連不上要在 15 秒內判定四種情況，並換三組傳輸參數送檔、試一次預先準備 |
