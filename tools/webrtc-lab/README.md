...
並行連線數預設 6 條。2026-09-25 iPhone 15 Pro 對 Mac Chrome 154 的第一輪實測，iPhone 送出 100 MB 時，1 條每秒 4.8 MB、3 條 16.2 到 17.4 MB，4 通道 · 1 連線是 4.6 MB。Mac 送出時單條就有每秒 14 到 26 MB（20 MB），3 條 · 4 通道送 100 MB 是每秒 51 到 53 MB。第二輪在家用 Wi-Fi 6（5 GHz）上，iPhone 送出 3 條每秒 17.3 到 18.7 MB、4 條 20.0 到 23.2 MB、6 條 31.7 到 35.6 MB，RTT 5 到 17 ms。Mac 送出 3、4、6 條都停在每秒 52 到 55 MB。選中的候選對另外記下兩端位址的類別（`localNet`、`remoteNet`），分辨流量走區網還是 Tailscale。Chrome 在沒有相機權限時不給本機位址，遠端的 mDNS 與 prflx 候選常常也不給，這些情況記成 `unknown`。所以回送檢查裡是 `unknown`，接假攝影機、已授權相機的 `run-qr-camera.mjs` 裡才分得出 `lan`。
