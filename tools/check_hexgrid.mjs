...
// 另外補一輪。這幾條盯的就是那個修正，回頭有人改判定順序會在這裡轉紅。
{
  ...
  // 對岸要維持中國，補格補過頭會在這裡現形。
  //
  // 座標取本土不取島，而且搜尋半徑放到 0.35 度：10m 的海岸線精確到 2 公里，
  // 廈門島與本土之間那條水道真的會判成海，而 level 8 一格 17 公里，沿海城市
  // 附近的格心經常落在水上。要問的是「這一帶有沒有中國的格子」，不是「正上方有沒有」。
  for (const [nm, lat, lon] of [['廈門', 24.52, 118.12], ['福州', 26.074, 119.296]]) {
    check(has(lat, lon, 'cn', 0.35), `level 8 的 ${nm} 一帶有中國的格子`);
...
// 用手上那份，兩邊呼叫的是 hexgrid.js 的同一支 judgeAt，這裡驗它的行為。
{
  ...
  for (const [nm, lat, lon, want] of [
    ...
    ['台中', 24.15, 120.75, 'tw'], ['廈門', 24.52, 118.12, 'cn'],
  ...
  // 10m 的國界已經把金門歸給台灣，馬祖那幾座小島仍然沒收錄，所以縣市界那份還是要
  check(hex.judgeAt(bare, 24.432, 118.317) === 'tw', '10m 的國界把金門歸給台灣');
  check(hex.judgeAt(idx, 26.152, 119.937) === 'tw' && hex.judgeAt(bare, 26.152, 119.937) === null,
        '馬祖要靠縣市界那份才判得成台灣，只有國界的話是海');
